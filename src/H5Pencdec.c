...
/*--------------------------------------------------------------------------
 ...
 PURPOSE
    ...
    Multithread safe version that uses the multithread safe structures.
...
/*-------------------------------------------------------------------------
 ...
 PURPOSE
    ...
    Multithread safe version using the multithread safe structures.
...
H5P__decode(const void *buf)
{
    ...
    while (p) {
        ...
        if (prop_value.size > value_buf_size) {
            if (NULL == (value_buf = H5MM_realloc(value_buf, prop_value.size)))
                HGOTO_ERROR(H5E_PLIST, H5E_CANTALLOC, FAIL, "decoding buffer allocation failed");
            value_buf_size = prop_value.size;
        } /* end if */
