...
H5P_init_phase1(void)
{
    ...
    if ( 0 > H5P_mt_init_free_lists() )
    {
        HGOTO_ERROR(H5E_PLIST, H5E_CANTINIT, FAIL, 
                    "unable to initialize H5P MT safe free lists");
    }
#endif
            ...
            if (*lib_class->class_id >= 0) 
            {
                ...
                if (H5I_dec_ref(*lib_class->class_id) < 0)
                {
                    HDONE_ERROR(H5E_PLIST, H5E_CLOSEERROR, FAIL, 
                        "unable to close property list class ID");
                }
            }
 ...
 *              property in a property list and proerty list class respectively. 
 * 
...
H5P__find_prop_plist(H5P_genplist_t *plist, const char *name)
{
    ...
    if ( NULL == (ret_value = H5P__mt_search__list(plist, name)) )
    {
                   ...
                   H5P_prp_compare_func_t prp_cmp, H5P_prp_close_func_t prp_close)
{
    ...
    if (( ret_value = H5P__mt_ins_or_mod_prop__lfsll_ins(pl_head, new_prop, 
                                                         &deletes, &visited, 
                                                         &thrd_cols, &chksum_cols)) < 0 )
    {
              ...
              H5P_prp_close_func_t prp_close)
{
    ...
    if ((ret_value = H5P__mt_ins_or_mod_prop__main(pclass, name, def_value, size, FALSE, FALSE, 
                                                   ...
                                                   prp_decode, prp_delete, prp_copy, prp_cmp, 
                                                   prp_close)) < 0 )
#else
    if ((ret_value = H5P__mt_ins_or_mod_prop__class(pclass, name, def_value, size, prp_create, 
                                                    prp_set, prp_get, prp_encode, 
                                                    prp_decode, prp_delete, prp_copy, prp_cmp, 
                                                    prp_close)) < 0 )
#endif
...
#else
    if (( ret_value = H5P__mt_ins_or_mod_prop__list(plist, name, value, size, FALSE, FALSE,
                                                    NULL, prp_set, prp_get, prp_encode, prp_decode, 
                                                    prp_delete, prp_copy, prp_cmp, prp_close)) < 0 )
#endif
    {
...
H5P_poke(H5P_mt_list_t *plist, const char *name, void *value)
{
    ...
    if ( 0 > H5P__mt_ins_or_mod_prop__main(plist, name, value, prop_value.size, 
                                           ...
                                           prop->close) )
#else
    if ( 0 > H5P__mt_ins_or_mod_prop__list(plist, name, value, prop_value.size, 
                                           FALSE, FALSE, prop->create, prop->set, 
                                           prop->get, prop->encode, prop->decode, 
                                           prop->del, prop->copy, prop->cmp, 
                                           prop->close) )
...
#else
    if ( 0 > H5P__mt_ins_or_mod_prop__list(plist, name, (void *)prop_value.ptr, 
                                           prop_value.size, FALSE, FALSE, prop->create, 
                                           prop->set, prop->get, prop->encode, prop->decode, 
                                           prop->del, prop->copy, prop->cmp, prop->close) )
#endif
...
H5P__class_set(H5P_genclass_t *pclass, const char *name, void *value)
{
    ...
    if ( (H5P__mt_ins_or_mod_prop__main(pclass, name, value, prop_value.size,
                                        ...
                                        prop->close)) < 0 )
#else
    if ( (H5P__mt_ins_or_mod_prop__class(pclass, name, value, prop_value.size,
...
H5P__cmp_class(H5P_genclass_t *pclass1, H5P_genclass_t *pclass2)
{
    ...
    if ( 0 > ( ret_value = H5P__mt_cmp_class(pclass1, pclass2) ) )
    {
...
H5P__cmp_plist(H5P_genplist_t *plist1, H5P_genplist_t *plist2, int *cmp_ret)
{
    ...
    if ( 0 > ( *cmp_ret = H5P__mt_cmp_list(plist1, plist2) ) )
    {
        HGOTO_ERROR(H5E_PLIST, H5E_CANTCOMPARE, FAIL, 
...
H5P_class_isa(H5P_mt_class_t *pclass1, H5P_mt_class_t *pclass2)
{
    ...
    if ( ( equal = H5P__mt_cmp_class(pclass1, pclass2)) == 0 )
    {
        HGOTO_DONE(TRUE);
    }
    else 
...
H5P__copy_prop_plist(hid_t dst_id, hid_t src_id, const char *name)
{
    ...
    if ( NULL == ( src_prop = H5P__mt_search__list(src_plist, name) ))
    {
    ...
    if ( (dst_prop = H5P__mt_search__list(dst_plist, name)) != NULL )
    {
        assert(dst_prop);
        ...
        if ( 0 < H5P__mt_ins_or_mod_prop__main(dst_plist, prop->name, value.ptr, value.size,
                                               ...
                                               prop->del, prop->copy, prop->cmp, prop->close) )
#else
        if ( 0 < H5P__mt_ins_or_mod_prop__list(dst_plist, src_prop->name, value.ptr, value.size,
                                               FALSE, TRUE, src_prop->create, src_prop->set, 
                                               src_prop->get, src_prop->encode, src_prop->decode, 
                                               src_prop->del, src_prop->copy, src_prop->cmp, 
                                               src_prop->close) )
#endif
        {
...
#else
        if ( 0 < H5P__mt_ins_or_mod_prop__list(dst_plist, src_prop->name, value.ptr, value.size,
                                               TRUE, FALSE, src_prop->create, src_prop->set, 
                                               src_prop->get, src_prop->encode, src_prop->decode, 
                                               src_prop->del, src_prop->copy, src_prop->cmp, 
                                               src_prop->close) )
#endif
...
H5P__copy_prop_pclass(hid_t dst_id, hid_t src_id, const char *name)
{
    ...
    if ( NULL == (prop = H5P__mt_search__class(src_pclass, name)))
    {
    ...
    if ( 0 < H5P__mt_ins_or_mod_prop__main(dst_pclass, prop->name, value.ptr, value.size,
                                           ...
                                           prop->del, prop->copy, prop->cmp, prop->close) )
#else
    if ( 0 < H5P__mt_ins_or_mod_prop__class(dst_pclass, prop->name, value.ptr, value.size,
                                            prop->create, prop->set, 
                                            prop->get, prop->encode, prop->decode, 
                                            prop->del, prop->copy, prop->cmp, prop->close) )
#endif
    {
        ...
        if (par_path != NULL) 
        {
            ...
            if (NULL == (ret_value = (char *)H5MM_malloc(ret_str_len)))
            {
                HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, NULL, 
                            "memory allocation failed for class name");
            }
...
H5P__new_plist_of_type(H5P_plist_type_t type)
{
    ...
    if (type == H5P_TYPE_USER)
    {
        HGOTO_ERROR(H5E_PLIST, H5E_BADVALUE, H5I_INVALID_HID, 
                    "can't create user property list");
    }
...
H5P_get_class(H5P_genplist_t *plist)
{
    ...
    do
    {
        ...
        if ( ref_count.deleted )
        {
            ...
            if ( ! atomic_compare_exchange_strong(&(pclass->ref_count),
                                                    &ref_count, local_rc) )
            {
