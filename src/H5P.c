...
/*--------------------------------------------------------------------------
 NAME
    H5Pcreate
 PURPOSE
    No changes in H5Pcreate() for the multithread version
...
/*--------------------------------------------------------------------------
 ...
 DESCRIPTION
    ...
    zero-sized property MUST be set to NULL.  The property 'create' & 'close'
...
/*--------------------------------------------------------------------------
 NAME
    H5Pinsert2
 PURPOSE
    No changes in H5Pinsert2() for the multithread version
...
/*--------------------------------------------------------------------------
 NAME
    H5Pset
 PURPOSE
    No changes in H5Pset() for the multithread version
...
/*--------------------------------------------------------------------------
 NAME
    H5Pget_size
 PURPOSE
    Only changes are update to the multithread H5P structures
...
H5Pget_size(hid_t id, const char *name, size_t *size /*out*/)
{
    ...
    if (H5I_GENPROP_LST == H5I_get_type(id)) {
        ...
        if ((ret_value = H5P__get_size_pclass(pclass, name, size)) < 0)
            HGOTO_ERROR(H5E_PLIST, H5E_CANTREGISTER, FAIL, "unable to query size in pclass");
    }
    else {
        HGOTO_ERROR(H5E_ARGS, H5E_BADTYPE, FAIL, "not a property object");
    }
...
/*--------------------------------------------------------------------------
 NAME
    H5Pencode2
 PURPOSE
    No changes were made for the multithread version.
...
/*--------------------------------------------------------------------------
 NAME
    H5Pget_class
 PURPOSE
    Multithread version of H5Pget_class().
...
/*--------------------------------------------------------------------------
 NAME
    H5Piterate
 PURPOSE
    No changes were made for multithread H5P, however due to the structure
    of the multithread H5P structures, the functionality of this function has
    been moved to other functions, and currently only is being used because
    some existing tests specifically call it.
...
/*--------------------------------------------------------------------------
 ...
 DESCRIPTION
    ...
    NOTE: the only changes for the multithread version is adding an assert
    for debugging safety checking of the multithread H5P H5P_mt_list_t tag
    field.
...
/*--------------------------------------------------------------------------
 NAME
    H5Punregister
 PURPOSE
    Only changes were to update to use multithread H5P structures
...
/*--------------------------------------------------------------------------
 NAME
    H5Pclose
 PURPOSE
    No changes were made for mulithread H5P.
...
/*--------------------------------------------------------------------------
 NAME
    H5Pget_class_parent
 PURPOSE
    Multithread version of H5Pget_class_parent()
