 ...
 *
 *          NOTE: In the multithread version of H5P the function H5P__mt_is_equal() is
 *          used instead of H5P__cmp_prop().
 *
 *          NOTE: Also, in the multithread version H5P__mt_is_equal() returns zero if the
 ...
 *      H5Pget_nprops() in H5P.c and H5P_get_nprops_pclass() in H5Pint.c. The original
 *      version of H5P new classes do not inherit the parent's properties and instead
 *      will loop up to the parent class to search their properties if needed. Because of
 ...
 *
 * log_pl_len (_Atomic size_t):
 *      Number of valid properties defined in the property list at the current version.
 *
...
#endif
herr_t         H5P__set_delete_version(void *param, const char *name);
H5P_mt_prop_t *H5P__mt_search_prop(void *param, const char *name);

H5P_mt_list_table_entry_t *H5P__mt_search_lkup_tbl(H5P_mt_list_table_entry_t *lkup_tbl, size_t left_entry,
                                                   size_t right_entry, int64_t chksum);
...
herr_t         H5P__find_mod_point(H5P_mt_prop_t *pl_head, H5P_mt_prop_t **first_ptr_ptr,
                                   ...
                                   uint32_t *thrd_cols_ptr, int64_t chksum, uint64_t version);
