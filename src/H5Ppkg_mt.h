 ...
 *      Due to curr being updated to point to and store the version number of the most
 *      current version of property curr points to, the LFSLL may be iterated for no
 *      reason. This would happen if the version of the list we are searching in is older
 *      than curr.ver the LFSLL must be iterated to check if there is an older version of
 *      the property that is valid for the version we are searching for. If there winds
 *      up not being a valid property for the version we are in, then the base property
 *      must be used.
 *
 *      The atomic uint64_t first_ver_of_curr eliminates that needless iteration, by
 *      comparing to the version we are searching in. If first_ver_of_curr is greater
 *      than the version we need, we know there is no valid version in the LFSLL and we
 *      must use the base property, and no longer need to iterate the LFSLL to check.
 *
 ****************************************************************************************
 */
...
H5P_mt_class_t *H5P__mt_create_class(H5P_mt_class_t *parent, const char *name, H5P_plist_type_t type,
                                     uint64_t test_version, H5P_cls_create_func_t create_func,
...
herr_t H5P__mt_ins_or_mod_prop__main(void *param, const char *name, void *value, size_t size, bool create,
...
H5P_mt_prop_t *H5P__mt_search_prop(void *param, int64_t chksum, const char *name);

H5P_mt_list_table_entry_t *H5P__mt_search_lkup_tbl(H5P_mt_list_table_entry_t *lkup_tbl, uint32_t left_entry,
                                                   uint32_t right_entry, int64_t chksum);

H5P_mt_prop_t *H5P__mt_search_lfsll(H5P_mt_prop_t *pl_head, int64_t chksum, uint64_t version);
...
herr_t H5P__find_mod_point(H5P_mt_prop_t *pl_head, H5P_mt_prop_t **first_ptr_ptr,
                           ...
                           uint32_t *thrd_cols_ptr, H5P_mt_prop_t *target_prop);

H5P_mt_prop_t *H5P__get_next_valid_prop(H5P_mt_prop_t *prop, uint64_t version);

H5P_mt_prop_t *H5P__find_valid_version(H5P_mt_prop_t *prop, uint64_t version);

int32_t H5P__is_valid(H5P_mt_prop_t *prop, uint64_t version);

int32_t H5P__mt_compare_prop(H5P_mt_prop_t *prop1, H5P_mt_prop_t *prop2);

int32_t H5P__mt_is_equal(H5P_mt_prop_t *prop1, H5P_mt_prop_t *prop2);

int32_t H5P__mt_cmp_list_or_class(void *param1, void *param2);
...
uint64_t H5P__mt_version_check(void *param, uint64_t curr_version, uint64_t next_version);
...
herr_t H5P__mt_get_value(H5P_mt_list_t *list, int64_t chksum, const char *name, void *value_ptr);
