 ...
 * conditions, the size and pointer must be set atomically.  This structure exists to
 * facilitate this.
 *
 * Fields:
 *
 ...
 * so for this reason, the (name, create_version) key is augmented with a 32 bit
 ...
 * field, allows us to operate on specific versions of property list classes and
 ...
 *
 *          NOTE: in H5P_mt_prop_t, the value is located in the field
 ...
 *
 * The possibility of accessing a property list class or property list that is in the
 * process of being initialized stems from two points.
 *
 ...
 * on entry, and if the opening flag is set to TRUE must wait until it's FALSE.
 *
 ...
 * returned ID into the class's id field. The reason for this is to follow the same
 * process as the original non-multithread H5P code for an easier transition for the new
 * H5P multithread structures and functions.
 *
 ...
 *
 * H5P__unregister__num_calls (_Atomic uint64_t):
 *      Tracks the number of H5P__unregister() function calls.
 *
 ...
 *
 * num_delete_prop__cols (_Atomic uint64_t):
 *      The number of collisions with other threads that occur when attempting to
 *      atomically set the delete_version of a property.
 *
 * num_delete_prop__success (_Atomic uint64_t):
 ...
 *
 * num_delete_prop__chksum_cols (_Atomic uint64_t):
 ...
 * Search statistics:
 *
 * H5P__mt_search_prop__class__num_calls (_Atomic uint64_t):
 *      Tracks the number of H5P__mt_search_prop__class() function calls.
 *
...
typedef struct H5P_mt_class_t {
    ...
    _Atomic uint64_t H5P__register_real__num_calls;
    _Atomic uint64_t H5P__register__num_calls;
    _Atomic uint64_t H5P__mt_ins_or_mod_prop__class__num_calls;
    _Atomic uint64_t H5P__class_set__num_calls;
    ...
    _Atomic uint64_t H5P__unregister__num_calls;
    _Atomic uint64_t delete_prop__max_nodes_visited;
    _Atomic uint64_t delete_prop__avg_nodes_visited;
    _Atomic uint64_t num_delete_prop__nodes_visited;
    _Atomic uint64_t num_delete_prop__cols;
    _Atomic uint64_t num_delete_prop__success;
    _Atomic uint64_t num_delete_prop__chksum_cols;

    /* H5P_mt_class_t search stats */
    _Atomic uint64_t H5P__mt_search_prop__class__num_calls;
 ...
 * insert_avg_nodes_visited (_Atomic uint64_t):
 *      This field tracks the the average number of nodes visited in the LFSLL of the
 *      list across all inserts. Fields H5P_insert__num_calls and
 ...
 *      H5P__set_delete_version().
 *
 * num_delete_prop_chksum_cols (_Atomic uint64_t):
 *      The number of properties that have their delete_version set that have another
 *      version of the property in the LFSLL of the list.
 *
 * num_delete_prop__base_delete_version (_Atomic uint64_t):
...
typedef struct H5P_mt_list_t {
    ...
    _Atomic uint64_t H5P_insert__num_calls;
    _Atomic uint64_t H5P__mt_ins_or_mod_prop__class__num_calls;
    _Atomic uint64_t H5P_poke__num_calls;
    _Atomic uint64_t H5P_set__num_calls;
    _Atomic uint64_t insert_max_nodes_visited;
    _Atomic uint64_t insert_avg_nodes_visited;
    _Atomic uint64_t num_insert_nodes_visited;
    ...
    _Atomic uint64_t num_insert_prop__chksum_cols;
    _Atomic uint64_t num_set_new_value_cols;
    _Atomic uint64_t num_set_new_value;

    /* H5P_mt_list_t set delete version stats */
    _Atomic uint64_t H5P_remove__num_calls;
    _Atomic uint64_t H5P__mt_delete_prop__list__num_calls;
    _Atomic uint64_t num_delete_props_from_lfsll;
    _Atomic uint64_t delete_prop__max_nodes_visited;
    _Atomic uint64_t delete_prop__avg_nodes_visited;
    _Atomic uint64_t num_delete_prop__nodes_visited;
    _Atomic uint64_t num_delete_prop__cols;
    _Atomic uint64_t num_delete_prop__success;
    _Atomic uint64_t num_delete_prop_chksum_cols;
    _Atomic uint64_t num_delete_prop__base_delete_version;
    _Atomic uint64_t num_delete_prop__curr_entry;
    _Atomic uint64_t num_delete_prop__older_curr;

    /* H5P_mt_list_t search stats */
    _Atomic uint64_t H5P__find_prop_plist__num_calls;
    _Atomic uint64_t H5P__mt_search_prop__list__num_calls;
 ...
 *
 * class_fl_head_update (_Atomic uint64_t):
 *      Number of times the head of the class free list was updated.
 *
 * class_fl_head_update_cols (_Atomic uint64_t):
 *      Number of times a collision occured with another thread when trying to update the
 *      head of the class free list.
 *
 * class_fl_tail_update (_Atomic uint64_t):
 *      Number of times the tail of the class free list was updated.
 *
 * class_fl_tail_update_cols (_Atomic uint64_t):
 *      Number of times a collision occured with another thread when trying to update the
 *      tail of the class free list.
 *
 * class_fl_next_update (_Atomic uint64_t):
 *      Number of times the fl_next.ptr of a class on the class free list was updated.
 *
 ...
 *
 * list_fl_head_update_cols (_Atomic uint64_t):
 *      Number of times a collision occured with another thread when trying to update the
 *      head of the list free list.
 *
 * list_fl_tail_update (_Atomic uint64_t):
 *      Number of times the tail of the list free list was updated.
 *
 * list_fl_tail_update_cols (_Atomic uint64_t):
 *      Number of times a collision occured with another thread when trying to update the
 *      tail of the list free list.
 *
 * list_fl_next_update (_Atomic uint64_t):
 *      Number of times a the fl_next.ptr of a list on the list free list gets
 *      updated.
 *
 ...
 *      NOTE: this means if this thread is modifying the list, or if another thread is
...
typedef struct H5P_mt_t {
    ...
    _Atomic uint64_t H5P__create_class__num_calls;
    _Atomic uint64_t H5P__copy_pclass__num_calls;
    _Atomic uint64_t H5P__mt_create_class__internal__num_calls;
    _Atomic uint64_t num_classes_created_wo_parent;
    ...
    _Atomic uint64_t H5P_create_id__num_calls;
    _Atomic uint64_t H5P__create_list__num_calls;
    _Atomic uint64_t H5P_copy_plist__num_calls;
    _Atomic uint64_t H5P__mt_create_list__internal__num_calls;
    ...
    _Atomic uint64_t H5P__copy_prop_plist__num_calls;
    _Atomic uint64_t H5P__copy_prop_pclass__num_calls;
    _Atomic uint64_t num_prop_structs_allocated_from_heap;
    _Atomic uint64_t num_prop_structs_allocated_from_fl;
    _Atomic uint64_t num_props_created_wo_cbs;
    ...
    /* stats for property deletes (setting delete_versions) */
    ...
    /* Default list stats */
    _Atomic uint64_t num_default_list_mods;

    /* stats for getting list version from context */
    _Atomic uint64_t num_list_version_from_ctx;
    ...
    _Atomic uint64_t num_classes_loop_enforce_serial;
    _Atomic uint64_t num_lists_loop_enforce_serial;

    /* stats for closing classes */
    _Atomic uint64_t H5P__close_class_cb__num_calls;
    _Atomic uint64_t H5P__close_class__num_calls;
    ...
    _Atomic uint64_t H5P__close_list_cb__num_calls;
    _Atomic uint64_t H5P_close__num_calls;
...
H5P_mt_class_t *H5P__mt_copy_class(H5P_mt_class_t *og_class);
H5P_mt_class_t *H5P__mt_create_class__internal(H5P_mt_class_t *parent, const char *name,
                                               H5P_plist_type_t type, uint64_t src_version,
...
H5P_mt_list_t  *H5P__mt_create_list(H5P_mt_class_t *parent, uint64_t src_version, bool app_ref);
H5P_mt_list_t  *H5P__mt_copy_list(H5P_mt_class_t *parent, H5P_mt_list_t *og_list, uint64_t src_version,
                                  bool app_ref);
H5P_mt_list_t  *H5P__mt_create_list__internal(H5P_mt_class_t *parent, uint64_t src_version);
...
herr_t H5P__mt_ins_or_mod_prop__class(H5P_mt_class_t *class, const char *name, void *value, size_t size,
                                      bool is_new, uint64_t prop_version, H5P_prp_create_func_t prp_create,
...
herr_t H5P__mt_delete_prop__list(H5P_mt_list_t *list, int64_t chksum, const char *name, uint64_t curr_version,
...
herr_t H5P__count_nprops_pclass(H5P_mt_class_t *class, size_t *_nprops, uint64_t version, hbool_t recurse);
