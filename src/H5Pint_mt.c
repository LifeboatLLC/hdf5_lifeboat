                               ...
                               H5P_prp_close_func_t prp_close)
{
    ...
    uint64_t            max_visited   = 0; /* Stats variable */
    ...
    /* Insert the property into the LFSLL */
    H5P__mt_ins_or_mod_prop__lfsll_ins(pl_head, new_prop, 
                                       &deletes, &visited, &thrd_cols);
    ...
    max_visited = atomic_load(&(class->insert_max_nodes_visited));
    ...
    avg_visited = avg_visited + ( (visited - avg_visited ) / (num_calls + 1) );
                              ...
                              H5P_prp_close_func_t prp_close)
{
    ...
    /** 
     * If the in_lkup_tbl flag is FALSE then check if the next prop in the lfsll 
     * has the same chksum then don't increment logical length, because this is a 
     * 'modification' to an existing prop and not an entirely property.
     */
    if ( new_prop->in_lkup_tbl == FALSE )
    {
        next = atomic_load(&(new_prop->next));
        next_prop = next.ptr;
    ...
    max_visited = atomic_load(&(list->insert_max_nodes_visited));
                                   ...
                                   uint32_t *thrd_cols_ptr)
{
    ...
    do
    {
        ...
        /* Check to make sure there isn't collision of chksums with different names */
 ...
 */
    ...
    num_calls   = atomic_load(&(list->H5P__delete_prop__list__num_calls));
...
H5P__mt_search__class(H5P_mt_class_t *class, const char *name)
{
    ...
    if ( NULL == (prop = H5P__mt_search_lfsll(pl_head, chksum, curr_version)) )
    {
 ...
 *
 *              Called by H5P__mt_cmp_list_or_class() this function ensures that when 
...
H5P__mt_next_prop_to_cmp(H5P_mt_prop_t *prop, uint64_t version)
{
    ...
    if ( NULL == (valid_prop = H5P__get_next_valid_prop(prev_prop, version)) )
    {
        HGOTO_DONE(NULL);
    }
    if ( valid_prop->in_lkup_tbl )
...
H5P__mt_close_list(H5P_mt_list_t * list)
{
    ...
    do 
    {
        ...
        valid_prop = H5P__get_next_valid_prop(prop, list_version);
...
H5P__init_stats_class(H5P_mt_class_t *class)
{
    ...
    atomic_init(&(class->num_ref_count_deleted_true),         0ULL);
    atomic_init(&(class->num_ref_count_deleted_false),        0ULL);
...
H5P__reset_stats_class(H5P_mt_class_t *class)
{
    ...
    atomic_store(&(class->num_ref_count_deleted_true),         0ULL);
    atomic_store(&(class->num_ref_count_deleted_false),        0ULL);
...
H5P__init_stats_list(H5P_mt_list_t *list)
{
    ...
    //atomic_init(&(list->num_search_list__found_older_curr), 0ULL);
...
H5P__reset_stats_list(H5P_mt_list_t *list)
{
    ...
    //atomic_store(&(list->num_search_list__found_older_curr), 0ULL);
...
H5P__dump_stats_class(FILE *file_ptr, H5P_mt_class_t *class)
{
    ...
    fprintf(file_ptr, "class->num_target_prop_found_but_deleted                 = %lld\n", 
        (unsigned long long)(atomic_load(&(class->num_target_prop_found_but_deleted))));
...
H5P__mt_term_free_lists(void)
{
    ...
    {
        HGOTO_ERROR(H5E_VFL, H5E_BADTYPE, FAIL, "Not all H5P objects have been closed");
