 ...
 *
 *              NOTE: Once initialized, the free lists will always contain one entry, and
 *              are logically empty if prop_fl_head == prop_fl_tail != NULL (same for the
 *              class and list free lists). Thus a first blank structure is initialized
 *              to act as the head and tail of the free lists, until another object is
 *              inserted into them.
 *
 * Return:      SUCCEED/FAIL
 *
...
H5P_mt_init_free_lists(void)
{
    ...
    /**
     * Initialize the fields of H5P_mt_g, which is the global struct for H5P
     * to handle the class free list, list free list, and prop free list, and
     * does some stat tracking.
     */
 ...
 */
H5P_mt_class_t *
H5P__mt_create_class(H5P_mt_class_t *parent, const char *name, H5P_plist_type_t type, uint64_t test_version,
 ...
 */
H5P_mt_list_t *
H5P__mt_create_list(H5P_mt_class_t *parent, H5P_mt_list_t *og_list, bool copy, uint64_t test_version,
                    bool app_ref)
{
         ...
         */
            ...
            if (old_base.ptr && (0 == old_base_delete || version < old_base_delete)) {
                ...
                do {
                    ...
                    if (!atomic_compare_exchange_strong(&(old_prop->ref_count), &old_prop_ref_count,
                                                        new_prop_ref_count)) {
                        /* failed, update stats and try again */
...
{
    H5P_mt_type_t       tag;               /* Used to determine if a class or list */
 ...
 */
herr_t
H5P__mt_ins_or_mod_prop__main(void *param, const char *name, void *value, size_t size, bool create, bool copy,
                              ...
                              H5P_prp_close_func_t prp_close)
{
    ...
    H5P__mt_ins_or_mod_prop__lfsll_ins(pl_head, new_prop, &deletes, &nodes_visited, &thrd_cols);
    ...
    if (!class) {
        ...
        if (!copy) {
            entry =
                H5P__mt_search_lkup_tbl(list->lkup_tbl, 0, (list->nprops_inherited - 1), new_prop->chksum);
...
H5P__set_delete_version(void *param, const char *name)
{
    ...
    if (tag == CLASS_TAG) {
        ...
        /* Increment thread count */
        if ((H5P__inc_thrd_count(class)) < 0)
            HGOTO_ERROR(H5E_PLIST, H5E_BADTYPE, FAIL, "Couldn't increment parent's thread count.");
        else
            inc_thrd_flag = TRUE;
    ...
    do {
        ...
        while (!done) {
             ...
             */
            H5P__find_mod_point(pl_head, &prev_prop, &prop, &deletes, &nodes_visited, &thrd_cols, chksum,
                                curr_version);
...
H5P__mt_search_prop(void *param, const char *name)
{
    ...
    if (tag == CLASS_TAG) {
        ...
        assert(H5P_MT_ASSERT_FAIL);
        HGOTO_ERROR(H5E_PLIST, H5E_BADTYPE, NULL, "Type passed in wasn't a class or list.");
    }
 ...
 *              H5P__mt_search_lfsll() is used instead of this function.
 *              H5P__mt_search_lfsll() doesn't compare versions and always returns the
 *              most current version. If in the future specific versions will need to be
 *              searched for then this function will most likely replace
 *              H5P__mt_search_prop() for those specific searches.
 *
...
/**
 * NOTE: This is commented out because there is a test that compares two
 * lists (one is a copy of the other), and compares them equal. It passes.
 * However, it then modifies a property and compares them again, obviously
 * no longer equal.
 * Then modifies that property to the default value and expects the two
 * lists to still compare as equal. So comparing the bool in_prop_class,
 * bool in_lkup_tbl, and uint64_t ref_count cannot be done and still have
 * the existing hdf5 tests pass.
 */
#if 0
...
H5P__mt_cmp_list_or_class(void *param1, void *param2)
{
    ...
    if (tag1 == CLASS_TAG && tag2 == CLASS_TAG) {
        ...
        if ((cmp_value = HDstrcmp(class1->name, class2->name)) != 0)
            HGOTO_DONE(1);
/**
 * NOTE: Due to copying a class and then registering the copy with a new ID
 * the IDs of the classes can not be compared, else copies would fail when
 * the library expects them to succeed.
 */
#if 0
...
H5P__mt_get_value(H5P_mt_list_t *list, const char *name, void *value_ptr)
{
    ...
    prop = H5P__mt_search_prop(list, name);
...
H5P__mt_close_class(H5P_mt_class_t *class)
{
    ...
    if (ref_count.pl == 0 && ref_count.plc == 0 && ref_count.deleted) {
        ...
        /**
         * Atomically updates the current tail of the class's
 ...
 */
    ...
    for (uint32_t i = 0; i < list->nprops_inherited; i++) {
        ...
        /**
         * If the base.ptr of an entry isn't NULL, the property in the parent class
         * is being referenced and the ref_count on that property must be decremented.
         */
        if (base_ref.ptr) {
            base_prop = base_ref.ptr;
