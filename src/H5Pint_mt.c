 ...
 */
H5P_mt_list_t *
H5P__mt_create_list(H5P_mt_class_t *parent, H5P_mt_list_t *old_list, bool copy, uint64_t test_version,
                    bool app_ref)
{
...
H5P__set_delete_version(void *param, H5P_mt_prop_t *target_prop)
{
    ...
    do {
        if (!class) {
            entry =
                H5P__mt_search_lkup_tbl(list->lkup_tbl, 0, (list->nprops_inherited - 1), target_prop->chksum);
...
H5P__mt_search_lkup_tbl(H5P_mt_list_table_entry_t *lkup_tbl, uint32_t left_entry, uint32_t right_entry,
                        int64_t chksum)
{
    H5P_mt_list_table_entry_t *entry;
    int32_t                    left;
