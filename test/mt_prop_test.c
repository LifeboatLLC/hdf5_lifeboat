...
#if 0
test_op_info_t * 
prop_ver_check_list(H5P_mt_list_t *list, prop_table_entry_t *prop_entry, test_op_info_t *op_info,
                    uint64_t curr_ver, bool success);
#endif
...
search_list(thread_params_t *thread_params)
{
    ...
    if (list) {
        ...
        if (atomic_load(&(list->tag)) == H5P_MT_LIST_TAG) {
            ...
            if (ret == SUCCEED) {
                /**
                 * NOTE: Find the exact property we grabbed the value from
                 */
                assert(value > 0);
                ...
                for (size_t i = 0; i < list->nprops_inherited; i++) {
                    ...
                    if (lkup_entry->chksum == prop_entry->chksum) {
                        ...
                        /* If there isn't a curr in the lkup_entry */
                        if (first_curr == 0 || first_curr > op_info->obj_ver) {
                            ...
                            if (ret == 0) {
                                 ...
                                 * or was deleted after we searched
                                 */
                                ...
                                op_info->prop   = prop_ref.ptr;
                                op_info->result = OP_SUCCESS;
                                break;
                            }
                            /**
                             * The operation was not done on the base,
                             * must double check curr and version.
                             */
                            else {
                                if (first_curr > 0 && first_curr <= curr_ver) {
                        ...
                        } /* end if ( first_curr == 0 || first_curr > op_info->obj_ver ) */
                        /**
                         * The operation was not done on the base,
                         * must double check curr and version.
                         */
                ...
                /* If the operation was not done on the base of a lkup_entry */
                if (op_info->result == NOT_ATTEMPTED) {
                    /* If we don't have a prop pointer from a curr of a lkup_entry */
                    if (!prop) {
                    ...
                    /**
                     * Either the curr pointer we got from the lkup_tbl, or the version
                     * of the prop closest to curr_ver is the correct prop.
                     */
                    if (ret == 0) {
                        op_info->prop   = prop;
                    ...
                    /* The correct prop must be an older version, must iterate and check */
                    else {
                        curr_create_ver = atomic_load(&(prop->create_version));
                        create_ver      = curr_create_ver;

                        do {
                            next = atomic_load(&(prop->next));
                            prop = next.ptr;

                            assert(prop->chksum == prop_entry->chksum);

                            prop_value = atomic_load(&(prop->value));

                            ret = memcmp(prop_value.ptr, &value, prop_value.size);

                            /* This is the correct version of the prop */
                            if (ret == 0) {
                        ...
                        } while (op_info->result == NOT_ATTEMPTED);
...
#if 0
                op_info = prop_ver_check_list(list, prop_entry, op_info, curr_ver, TRUE);
                assert(op_info);
                assert(op_info->result == OP_SUCCESS);
                        ...
                        for (size_t i = 0; i < list->nprops_inherited; i++) {
                            ...
                            if (lkup_entry->chksum == prop_entry->chksum) {
                                ...
                                if (first_curr == 0 || first_curr > op_info->obj_ver) {
                                    ...
                                    if (base_del_ver <= op_info->obj_ver) {
                                        op_info->op_ver = op_info->obj_ver;
                                    }
                                    else if (base_del_ver <= curr_ver &&
                                             (first_curr == 0 || first_curr > curr_ver)) {
                                        op_info->op_ver = curr_ver;
                                    }
                                    else if (first_curr > 0 && first_curr <= curr_ver) {
                                        prop_ref = atomic_load(&(lkup_entry->curr));
                                        prop     = prop_ref.ptr;
                                        break;
                                    }
                        ...
                        if (op_info->result == NOT_ATTEMPTED) {
                            ...
                            if (create_ver > op_info->obj_ver) {
                                do {
                                    ...
                                    create_ver = atomic_load(&(next_prop->create_version));

                                    /**
                                     * A property did exist during the version we searched,
                                     * double check it was deleted.
                                     */
                                    if (create_ver <= op_info->obj_ver &&
                                        next_prop->chksum == prop_entry->chksum) {
                                        delete_ver = atomic_load(&(next_prop->delete_version));
                                        assert(delete_ver > 0 && delete_ver <= op_info->obj_ver);

                                        op_info->prop   = next_prop;
                                        op_info->op_ver = op_info->obj_ver;
                                        op_info->result = PROP_DELETED;
                                    }
                                    else if (next_prop->chksum != prop_entry->chksum) {
                                        delete_ver = atomic_load(&(prop->delete_version));

                                        if (delete_ver == 0 || delete_ver > curr_ver) {
                                            op_info->op_ver = op_info->obj_ver;
                                            op_info->result = PROP_DOESNT_EXIST;
                                        }
                                        else if (delete_ver <= curr_ver) {
                                            op_info->prop   = prop;
                                            op_info->op_ver = curr_ver;
                                            op_info->result = PROP_DELETED;
                                        }
                                    }
                                    else if (create_ver > op_info->obj_ver) {
                                        prop = next_prop;
                                    }

                                } while (op_info->result == NOT_ATTEMPTED);

                            } /* end if ( create_ver > op_info->obj_ver ) */
                            else {
                                delete_ver = atomic_load(&(prop->delete_version));

                                if (delete_ver <= op_info->obj_ver) {
                                    op_info->op_ver = op_info->obj_ver;
                                }
                                else if (delete_ver <= curr_ver) {
                                    op_info->op_ver = curr_ver;
...
#if 0
                        op_info = prop_ver_check_list(list, prop_entry, op_info, curr_ver, FALSE);
                        assert(op_info);
                     ...
                     */
                        ...
                        for (size_t i = 0; i < list->nprops_inherited; i++) {
                            ...
                            if (lkup_entry->chksum == prop_entry->chksum) {
                                ...
                                /* If there isn't a curr in the lkup_entry */
                                if (first_curr == 0 || first_curr > op_info->op_ver) {
                                    prop_ref = atomic_load(&(lkup_entry->base));
                                    prop     = prop_ref.ptr;

                                    base_del_ver = atomic_load(&(lkup_entry->base_delete_version));
                                    assert(base_del_ver <= op_info->op_ver);
                        ...
                        if (op_info->result == NOT_ATTEMPTED) {
                            ...
                            if (!prop) {
                                ...
                                assert(delete_ver <= op_info->op_ver);
    ...
    if (list) {
        ...
        if (ret == SUCCEED) {
            ...
            for (size_t i = 0; i < list->nprops_inherited; i++) {
                ...
                if (lkup_entry->chksum == prop_entry->chksum) {
                    first_curr = atomic_load(&(lkup_entry->first_ver_of_curr));
                    assert(first_curr > 0 && first_curr <= curr_ver);
            ...
            if (op_info->result == NOT_ATTEMPTED) {
                ...
                if (curr_create_ver == op_info->op_ver) {
                    op_info->prop   = prop;
                    op_info->result = OP_SUCCESS;
                }
                /* The correct prop must be an older version, must iterate and check */
                else if (curr_create_ver > op_info->op_ver) {
                    create_ver = curr_create_ver;
                    ...
                    do {
                        ...
                        if (next_prop->chksum == prop_entry->chksum && create_ver == op_info->op_ver) {
                            op_info->prop = next_prop;

                            done = TRUE;
                        }
                        else if (next_prop->chksum != prop_entry->chksum || create_ver < op_info->op_ver) {
                            create_ver = atomic_load(&(prop->create_version));

                            assert(create_ver <= curr_ver && create_ver > op_info->op_ver);

                            op_info->prop   = prop;
                            op_info->op_ver = create_ver;

                            done = TRUE;
                        }
                        else {
                            assert(next_prop->chksum == prop_entry->chksum);
                            prop = next_prop;
                        }
                ...
                } /* end else if ( curr_create_ver > op_info->op_ver ) */
...
#if 0
                    op_info = prop_ver_check_list(list, prop_entry, op_info, 
                                                    curr_ver, FALSE);
        ...
        if (ret == SUCCEED) {
                ...
                if (lkup_entry->chksum == prop_entry->chksum) {
                    ...
                    if (first_curr == 0 || first_curr > op_info->obj_ver) {
                        ...
                        if (base_del_ver > 0) {
                            ...
                            if (base_del_ver > op_info->obj_ver && base_del_ver <= curr_ver) {
                                op_info->prop   = prop_ref.ptr;
                                op_info->op_ver = base_del_ver;
                                op_info->result = OP_SUCCESS;
                                break;
                            }
                            else {
                    ...
                    } /* end if ( first_curr == 0 || first_curr > curr_ver ) */
                    /**
                     * The operation was not done on the base,
                     * must double check curr and version.
                     */
            ...
            if (op_info->result == NOT_ATTEMPTED) {
                ...
                do {
                    ...
                    if (delete_ver > op_info->obj_ver && delete_ver <= curr_ver) {
                        op_info->prop   = prop;
                        op_info->op_ver = delete_ver;
                        op_info->result = OP_SUCCESS;
                    }
                    else {
                        assert(next_prop->chksum == prop_entry->chksum);
                        ...
                        if (create_ver > op_info->obj_ver) {
                            do {
                                ...
                                if (create_ver <= op_info->obj_ver &&
                                    next_prop->chksum == prop_entry->chksum) {
                                    delete_ver = atomic_load(&(next_prop->delete_version));
                                    if (delete_ver > 0 && delete_ver <= op_info->obj_ver) {
                                        op_info->prop   = next_prop;
                                        op_info->op_ver = op_info->obj_ver;
                                        op_info->result = PROP_DELETED;
                                    }
                    ...
                    uint64_t curr_ver, bool success)
{
    ...
    if ( op_info->op == SEARCH_VER )
    {
        obj_ver = op_info->op_ver;
    }
    else
    {
        obj_ver = op_info->obj_ver;
    }
    ...
    for ( size_t i = 0; i < list->nprops_inherited; i++ )
    {
        ...
        if ( lkup_entry->chksum == prop_entry->chksum )
        {
            ...
            /* The lkup_tbl->base is the most recent version for our operation */
            if ( first_curr_ver == 0 || first_curr_ver > curr_ver )
            {
                base_del_ver = atomic_load(&(lkup_entry->base_delete_version));
                ...
                if ( base_del_ver > 0 )
                {
                    ...
                    /* If the op is deleting a prop and it was successful */
                    if ( success && op_info->op == MOD_DELETE_PROP )
                    {
                        ...
                        /** 
                         * How did we succeed at deleting the property if the property 
                         * wasn't deleted during the range of versions we have? 
                         * Something's wrong. Fail and investigate.
                         */
                        else
                        {
                            assert(FALSE);
                    ...
                    else if ( success && op_info->op == SEARCH )
                    {
                        assert(base_del_ver > obj_ver);
                    ...
                    else
                    {
                        ...
                        /** 
                         * The version we searched doesn't really matter, because we
                         * know there was an order of operations that occured, but this
                         * is attempted to get best estimate at which version this
                         * attempt occured for sorting the array of operations during
                         * check_version().
                         */
                        if ( base_del_ver <= obj_ver )
                        {
                        ...
                        {
                            /**
                             * No other option should be possible. If during any op other
                             * than MOD_DELETE_PROP, and the base of the lkup_tbl is the
                             * most recent version of the prop and it's not deleted at
                             * the version of the list we attempted, then the op should 
                             * have succeeded.
                             */
                            assert(FALSE);
                        }
                ...
                {
                    /**
                     * No other option should be possible. If the base of the lkup_tbl is 
                     * the most recent version of the prop and it's not deleted at the 
                     * version of the list we attempted, then the op should have 
                     * succeeded.
                     */
                    assert(FALSE);
                }
            ...
            /* The lkup_tbl->curr is the most recent version for our operation */
            else
            {
                prop_ref  = atomic_load(&(lkup_entry->curr));
                curr_prop = prop_ref.ptr;
                break;
            }
    ...
    if ( op_info->result == NOT_ATTEMPTED )
    {
        ...
        if ( ! curr_prop )
        {
            curr_prop = get_prop_from_lfsll(list->pl_head, prop_entry->name, curr_ver);
        }
        CHECK_PTR(curr_prop, "grabbed prop from lkup_tbl/get_prop_from_lfsll");
        assert(curr_prop);


        check_prop = curr_prop;
        curr_create_ver = atomic_load(&(curr_prop->create_version));
        create_ver = curr_create_ver;

        if ( curr_create_ver > obj_ver )
        {
            do
            {
                ...
                if ( success && op_info->op == MOD_MOD_PROP )
                {
                    ...
                    {
                        create_ver = atomic_load(&(check_prop->create_version));
                        done = TRUE;
                    }
                }
                else
            ...
            if ( create_ver > curr_ver )
            {
                op_info->op_ver = curr_ver;
                op_info->result = PROP_DOESNT_EXIST;
            }

        } /* end else if ( create_ver > op_info->obj_ver ) */

        if ( success && op_info->op == MOD_MOD_PROP )
        {
            op_info->prop   = check_prop;
            op_info->op_ver = create_ver;
            op_info->result = OP_SUCCESS;
        }
        else if ( create_ver <= curr_ver && op_info->result == NOT_ATTEMPTED )
        {
            delete_ver = atomic_load(&(check_prop->delete_version));
            ...
            else if ( op_info->op == MOD_DELETE_PROP )
            {
                assert( !success && create_ver > obj_ver);

                op_info->result = PROP_DOESNT_EXIST;
            }
            else if ( op_info->op == SEARCH )
            {
                op_info->prop = check_prop;
                ...
                else
                {
                    if ( create_ver > obj_ver )
                    {
                        op_info->op_ver = obj_ver;
                        op_info->result = PROP_DOESNT_EXIST;
                    }
                }
            }
            else if ( !success && op_info->op == SEARCH_VER )
            {
                assert(create_ver > obj_ver);

                op_info->result = PROP_DOESNT_EXIST;
            }
            else
            {
                /* No other option should be possible */
                assert(FALSE);
            }
        
        } /* end if ( create_ver <= curr_ver ) */
        else
        {
            assert(FALSE);
        }
