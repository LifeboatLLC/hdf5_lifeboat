        ...
        PART_BEGIN(H5Fget_obj_count_types)
        {
            ...
            if (check_open_obj_count(obj_count, 1) < 0) {
                ...
                printf("    number of open named datatypes (%ld) did not match %s expected number (1)\n",
                       obj_count, active_thread_ct > 1 ? "or exceed" : "");
                PART_ERROR(H5Fget_obj_count_types);
            }
...
H5_api_file_test_add(void)
{
    ...
    AddTest("test_file_open_overlap", MT_API_TEST_FUNC_OUTER(test_file_open_overlap), NULL,
            "overlapping file opens", NULL);
    ...
    AddTest("test_reopen_file", MT_API_TEST_FUNC_OUTER(test_reopen_file), NULL,
            "re-open of a file with H5Freopen", NULL);
    AddTest("test_close_file_invalid_id", MT_API_TEST_FUNC_OUTER(test_close_file_invalid_id), NULL,
            "H5Fclose with an invalid ID", NULL);
    AddTest("test_flush_file", MT_API_TEST_FUNC_OUTER(test_flush_file), NULL, "H5Fflush", NULL);
    AddTest("test_file_is_accessible", MT_API_TEST_FUNC_OUTER(test_file_is_accessible), NULL,
            "H5Fis_accessible", NULL);
    AddTest("test_file_property_lists", MT_API_TEST_FUNC_OUTER(test_file_property_lists), NULL,
            "file property list operations", NULL);
    AddTest("test_get_file_intent", MT_API_TEST_FUNC_OUTER(test_get_file_intent), NULL,
            "retrieval of file intent with H5Fget_intent", NULL);
    ...
    AddTest("test_file_mounts", MT_API_TEST_FUNC_OUTER(test_file_mounts), NULL, "file mounting/unmounting",
            NULL);
    AddTest("test_get_file_name", MT_API_TEST_FUNC_OUTER(test_get_file_name), NULL, "retrieval of file name",
            NULL);
}
