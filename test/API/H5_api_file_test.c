/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 * Copyright by The HDF Group.                                               *
 * All rights reserved.                                                      *
 *                                                                           *
 * This file is part of HDF5.  The full HDF5 copyright notice, including     *
 * terms governing use, modification, and redistribution, is contained in    *
 * the COPYING file, which can be found at the root of the source code       *
 * distribution tree, or in https://www.hdfgroup.org/licenses.               *
 * If you do not have access to either file, you may request a copy from     *
 * help@hdfgroup.org.                                                        *
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */

#include "H5_api_file_test.h"

static void print_file_test_header(TestParams_t *params);

static herr_t test_create_file(TestParams_t *params);
static herr_t test_create_file_invalid_params(TestParams_t *params);
static herr_t test_create_file_excl(TestParams_t *params);
static herr_t test_open_file(TestParams_t *params);
static herr_t test_open_file_invalid_params(TestParams_t *params);
static herr_t test_open_nonexistent_file(TestParams_t *params);
static herr_t test_file_open_overlap(TestParams_t *params);
static herr_t test_file_permission(TestParams_t *params);
static herr_t test_reopen_file(TestParams_t *params);
static herr_t test_close_file_invalid_id(TestParams_t *params);
static herr_t test_flush_file(TestParams_t *params);
static herr_t test_file_is_accessible(TestParams_t *params);
static herr_t test_file_property_lists(TestParams_t *params);
static herr_t test_get_file_intent(TestParams_t *params);
static herr_t test_get_file_obj_count(TestParams_t *params);
static herr_t test_file_mounts(TestParams_t *params);
static herr_t test_get_file_name(TestParams_t *params);
static herr_t check_open_obj_count(ssize_t obj_count, int expected);

static void
print_file_test_header(TestParams_t H5_ATTR_UNUSED *params)
{
    printf("\n");
    printf("**********************************************\n");
    printf("*                                            *\n");
    printf("*               API File Tests               *\n");
    printf("*                                            *\n");
    printf("**********************************************\n\n");
}

/*
 * Tests that a file can be created.
 */
static herr_t
test_create_file(TestParams_t *params)
{
    hid_t file_id           = H5I_INVALID_HID;
    char *prefixed_filename = NULL;

    /* Make sure the connector supports the API functions being tested */
    if (!(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_BASIC)) {
        printf("    API functions for basic file aren't supported with this connector\n");
        return SKIP;
    }

    if (prefix_test_filename(params, test_path_prefix, FILE_CREATE_TEST_FILENAME, &prefixed_filename) < 0) {
        printf("    couldn't prefix filename\n");
        goto error;
    }

    if ((file_id = H5Fcreate(prefixed_filename, H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        printf("    couldn't create file '%s'\n", prefixed_filename);
        goto error;
    }

    if (H5Fclose(file_id) < 0)
        TESTFRAME_TEST_ERROR(params);
    if (GetTestCleanup() && H5Fdelete(prefixed_filename, H5P_DEFAULT) < 0)
        TESTFRAME_TEST_ERROR(params);

    free(prefixed_filename);
    prefixed_filename = NULL;

    return SUCCEED;

error:
    H5E_BEGIN_TRY
    {
        H5Fclose(file_id);
        if (GetTestCleanup())
            H5Fdelete(prefixed_filename, H5P_DEFAULT);
    }
    H5E_END_TRY

    free(prefixed_filename);

    return FAIL;
}

/*
 * Tests that a file can't be created when H5Fcreate is passed
 * invalid parameters.
 */
static herr_t
test_create_file_invalid_params(TestParams_t *params)
{
    hid_t file_id           = H5I_INVALID_HID;
    char *prefixed_filename = NULL;

    /* Make sure the connector supports the API functions being tested */
    if (!(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_BASIC)) {
        printf("    API functions for basic file aren't supported with this connector\n");
        return SKIP;
    }

    if (prefix_test_filename(params, test_path_prefix, FILE_CREATE_INVALID_PARAMS_FILE_NAME,
                             &prefixed_filename) < 0) {
        printf("    couldn't prefix filename\n");
        goto error;
    }

    SUBTEST_BEGIN(params, "H5Fcreate with invalid file name")
    {
        H5E_BEGIN_TRY
        {
            file_id = H5Fcreate(NULL, H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
        }
        H5E_END_TRY

        if (file_id >= 0) {
            printf("    file was created with a NULL name!\n");
            H5Fclose(file_id);
            TESTFRAME_TEST_ERROR(params);
        }

        H5E_BEGIN_TRY
        {
            file_id = H5Fcreate("", H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
        }
        H5E_END_TRY

        if (file_id >= 0) {
            printf("    file was created with an invalid name of ''!\n");
            H5Fclose(file_id);
            TESTFRAME_TEST_ERROR(params);
        }
    }
    SUBTEST_END(params);

    SUBTEST_BEGIN(params, "H5Fcreate with invalid flags")
    {
        H5E_BEGIN_TRY
        {
            file_id = H5Fcreate(prefixed_filename, H5F_ACC_RDWR, H5P_DEFAULT, H5P_DEFAULT);
        }
        H5E_END_TRY

        if (file_id >= 0) {
            printf("    file was created with invalid flag H5F_ACC_RDWR!\n");
            H5Fclose(file_id);
            TESTFRAME_TEST_ERROR(params);
        }

        H5E_BEGIN_TRY
        {
            file_id = H5Fcreate(prefixed_filename, H5F_ACC_CREAT, H5P_DEFAULT, H5P_DEFAULT);
        }
        H5E_END_TRY

        if (file_id >= 0) {
            printf("    file was created with invalid flag H5F_ACC_CREAT!\n");
            H5Fclose(file_id);
            TESTFRAME_TEST_ERROR(params);
        }

        H5E_BEGIN_TRY
        {
            file_id = H5Fcreate(prefixed_filename, H5F_ACC_SWMR_READ, H5P_DEFAULT, H5P_DEFAULT);
        }
        H5E_END_TRY

        if (file_id >= 0) {
            printf("    file was created with invalid flag H5F_ACC_SWMR_READ!\n");
            H5Fclose(file_id);
            TESTFRAME_TEST_ERROR(params);
        }
    }
    SUBTEST_END(params);

    SUBTEST_BEGIN(params, "H5Fcreate with invalid FCPL")
    {
        H5E_BEGIN_TRY
        {
            file_id = H5Fcreate(prefixed_filename, H5F_ACC_TRUNC, H5I_INVALID_HID, H5P_DEFAULT);
        }
        H5E_END_TRY

        if (file_id >= 0) {
            printf("    file was created with invalid FCPL!\n");
            H5Fclose(file_id);
            TESTFRAME_TEST_ERROR(params);
        }
    }
    SUBTEST_END(params);

    free(prefixed_filename);
    prefixed_filename = NULL;

    return SUCCEED;

error:
    H5E_BEGIN_TRY
    {
        /* Attempt to remove the file if it ended up being created. */
        H5Fdelete(prefixed_filename, H5P_DEFAULT);

        H5Fclose(file_id);
    }
    H5E_END_TRY

    free(prefixed_filename);

    return FAIL;
}

/*
 * Tests that file creation will fail when a file is created
 * using the H5F_ACC_EXCL flag while the file already exists.
 */
static herr_t
test_create_file_excl(TestParams_t *params)
{
    hid_t file_id           = H5I_INVALID_HID;
    hid_t file_id2          = H5I_INVALID_HID;
    char *prefixed_filename = NULL;

    /* Make sure the connector supports the API functions being tested */
    if (!(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_BASIC)) {
        printf("    API functions for basic file aren't supported with this connector\n");
        return SKIP;
    }

    if (prefix_test_filename(params, test_path_prefix, FILE_CREATE_EXCL_FILE_NAME, &prefixed_filename) < 0) {
        printf("    couldn't prefix filename\n");
        goto error;
    }

    if ((file_id = H5Fcreate(prefixed_filename, H5F_ACC_EXCL, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        printf("    couldn't create first file\n");
        goto error;
    }

    /* Close the file */
    if (H5Fclose(file_id) < 0)
        TESTFRAME_TEST_ERROR(params);

    /* Try again with H5F_ACC_EXCL. This should fail because the file already
     * exists on disk from the previous steps.
     */
    H5E_BEGIN_TRY
    {
        file_id = H5Fcreate(prefixed_filename, H5F_ACC_EXCL, H5P_DEFAULT, H5P_DEFAULT);
    }
    H5E_END_TRY

    if (file_id >= 0) {
        printf("    created already existing file using H5F_ACC_EXCL flag!\n");
        goto error;
    }

    /* Test creating with H5F_ACC_TRUNC. This will truncate the existing file on disk. */
    if ((file_id = H5Fcreate(prefixed_filename, H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        printf("    couldn't truncate the existing file\n");
        goto error;
    }

    if (H5Fclose(file_id) < 0)
        TESTFRAME_TEST_ERROR(params);
    if (GetTestCleanup() && H5Fdelete(prefixed_filename, H5P_DEFAULT) < 0)
        TESTFRAME_TEST_ERROR(params);

    free(prefixed_filename);
    prefixed_filename = NULL;

    return SUCCEED;

error:
    H5E_BEGIN_TRY
    {
        H5Fclose(file_id);
        H5Fclose(file_id2);
        if (GetTestCleanup())
            H5Fdelete(prefixed_filename, H5P_DEFAULT);
    }
    H5E_END_TRY

    free(prefixed_filename);

    return FAIL;
}

/*
 * Tests that a file can be opened.
 */
static herr_t
test_open_file(TestParams_t *params)
{
    hid_t file_id = H5I_INVALID_HID;

    /* Make sure the connector supports the API functions being tested */
    if (!(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_BASIC)) {
        printf("    API functions for basic file aren't supported with this connector\n");
        return SKIP;
    }

    SUBTEST_BEGIN(params, "H5Fopen in read-only mode")
    {
        if ((file_id = H5Fopen(H5_API_TEST_FILENAME(params), H5F_ACC_RDONLY, H5P_DEFAULT)) < 0) {
            printf("    unable to open file '%s' in read-only mode\n", H5_API_TEST_FILENAME(params));
            TESTFRAME_TEST_ERROR(params);
        }
    }
    SUBTEST_END(params);

    if (file_id >= 0) {
        H5E_BEGIN_TRY
        {
            H5Fclose(file_id);
        }
        H5E_END_TRY
        file_id = H5I_INVALID_HID;
    }

    SUBTEST_BEGIN(params, "H5Fopen in read-write mode")
    {
        if ((file_id = H5Fopen(H5_API_TEST_FILENAME(params), H5F_ACC_RDWR, H5P_DEFAULT)) < 0) {
            printf("    unable to open file '%s' in read-write mode\n", H5_API_TEST_FILENAME(params));
            TESTFRAME_TEST_ERROR(params);
        }
    }
    SUBTEST_END(params);

    if (file_id >= 0) {
        H5E_BEGIN_TRY
        {
            H5Fclose(file_id);
        }
        H5E_END_TRY
        file_id = H5I_INVALID_HID;
    }

    /*
     * XXX: SWMR open flags
     */

    return SUCCEED;

error:
    H5E_BEGIN_TRY
    {
        H5Fclose(file_id);
    }
    H5E_END_TRY

    return FAIL;
}

/*
 * Tests that a file can't be opened when H5Fopen is given
 * invalid parameters.
 */
static herr_t
test_open_file_invalid_params(TestParams_t *params)
{
    hid_t file_id = H5I_INVALID_HID;

    /* Make sure the connector supports the API functions being tested */
    if (!(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_BASIC)) {
        printf("    API functions for basic file aren't supported with this connector\n");
        return SKIP;
    }

    SUBTEST_BEGIN(params, "H5Fopen with invalid file name")
    {
        H5E_BEGIN_TRY
        {
            file_id = H5Fopen(NULL, H5F_ACC_RDWR, H5P_DEFAULT);
        }
        H5E_END_TRY

        if (file_id >= 0) {
            printf("    file was opened with a NULL name!\n");
            H5Fclose(file_id);
            TESTFRAME_TEST_ERROR(params);
        }

        H5E_BEGIN_TRY
        {
            file_id = H5Fopen("", H5F_ACC_RDWR, H5P_DEFAULT);
        }
        H5E_END_TRY

        if (file_id >= 0) {
            printf("    file was opened with an invalid name of ''!\n");
            H5Fclose(file_id);
            TESTFRAME_TEST_ERROR(params);
        }
    }
    SUBTEST_END(params);

    SUBTEST_BEGIN(params, "H5Fopen with invalid flags")
    {
        H5E_BEGIN_TRY
        {
            file_id = H5Fopen(H5_API_TEST_FILENAME(params), H5F_ACC_TRUNC, H5P_DEFAULT);
        }
        H5E_END_TRY

        if (file_id >= 0) {
            printf("    file was opened with invalid flag H5F_ACC_TRUNC!\n");
            H5Fclose(file_id);
            TESTFRAME_TEST_ERROR(params);
        }

        H5E_BEGIN_TRY
        {
            file_id = H5Fopen(H5_API_TEST_FILENAME(params), H5F_ACC_EXCL, H5P_DEFAULT);
        }
        H5E_END_TRY

        if (file_id >= 0) {
            printf("    file was opened with invalid flag H5F_ACC_EXCL!\n");
            H5Fclose(file_id);
            TESTFRAME_TEST_ERROR(params);
        }
    }
    SUBTEST_END(params);

    return SUCCEED;

error:
    H5E_BEGIN_TRY
    {
        H5Fclose(file_id);
    }
    H5E_END_TRY

    return FAIL;
}

/*
 * A test to ensure that opening a file which doesn't exist will fail.
 */
static herr_t
test_open_nonexistent_file(TestParams_t *params)
{
    hid_t file_id           = H5I_INVALID_HID;
    char *prefixed_filename = NULL;

    /* Make sure the connector supports the API functions being tested */
    if (!(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_BASIC)) {
        printf("    API functions for basic file aren't supported with this connector\n");
        return SKIP;
    }

    if (prefix_test_filename(params, test_path_prefix, NONEXISTENT_FILENAME, &prefixed_filename) < 0) {
        printf("    couldn't prefix filename\n");
        goto error;
    }

    /* Make sure to first delete the file so we know for sure it doesn't exist */
    H5E_BEGIN_TRY
    {
        H5Fdelete(prefixed_filename, H5P_DEFAULT);
    }
    H5E_END_TRY;

    H5E_BEGIN_TRY
    {
        file_id = H5Fopen(prefixed_filename, H5F_ACC_RDWR, H5P_DEFAULT);
    }
    H5E_END_TRY

    if (file_id >= 0) {
        printf("    non-existent file was opened!\n");
        goto error;
    }

    free(prefixed_filename);
    prefixed_filename = NULL;

    return SUCCEED;

error:
    H5E_BEGIN_TRY
    {
        H5Fclose(file_id);
    }
    H5E_END_TRY

    free(prefixed_filename);

    return FAIL;
}

/*
 * Tests that a file can be opened read-only or read-write
 * and things are handled appropriately.
 */
static herr_t
test_file_permission(TestParams_t *params)
{
    hid_t  file_id           = H5I_INVALID_HID;
    hid_t  dset_id           = H5I_INVALID_HID;
    hid_t  dspace_id         = H5I_INVALID_HID;
    hid_t  group_id          = H5I_INVALID_HID;
    hid_t  attr_id           = H5I_INVALID_HID;
    hid_t  dtype_id          = H5I_INVALID_HID;
    char  *prefixed_filename = NULL;
    herr_t h5_ret            = FAIL;

    /* Make sure the connector supports the API functions being tested */
    if (!(vol_cap_flags_g & (H5VL_CAP_FLAG_FILE_BASIC)) || !(vol_cap_flags_g & H5VL_CAP_FLAG_GROUP_BASIC) ||
        !(vol_cap_flags_g & H5VL_CAP_FLAG_DATASET_BASIC) || !(vol_cap_flags_g & H5VL_CAP_FLAG_ATTR_BASIC) ||
        !(vol_cap_flags_g & H5VL_CAP_FLAG_STORED_DATATYPES)) {
        printf("    API functions for basic file, group, dataset, attribute, or stored datatype aren't "
               "supported with this connector\n");
        return SKIP;
    }

    if (prefix_test_filename(params, test_path_prefix, FILE_PERMISSION_TEST_FILENAME, &prefixed_filename) <
        0) {
        printf("    couldn't prefix filename\n");
        goto error;
    }

    if ((file_id = H5Fcreate(prefixed_filename, H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        printf("    couldn't create file '%s'\n", prefixed_filename);
        goto error;
    }

    if ((dspace_id = H5Screate(H5S_SCALAR)) < 0) {
        printf("    couldn't create data space\n");
        goto error;
    }

    if ((dset_id = H5Dcreate2(file_id, FILE_PERMISSION_TEST_DSET_NAME, H5T_STD_U32LE, dspace_id, H5P_DEFAULT,
                              H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        printf("    couldn't create data set: %s\n", FILE_PERMISSION_TEST_DSET_NAME);
        goto error;
    }

    if (H5Dclose(dset_id) < 0)
        TESTFRAME_TEST_ERROR(params);
    if (H5Fclose(file_id) < 0)
        TESTFRAME_TEST_ERROR(params);

    /* Open the file (with read-only permission) */
    if ((file_id = H5Fopen(prefixed_filename, H5F_ACC_RDONLY, H5P_DEFAULT)) < 0) {
        printf("    couldn't open file\n");
        goto error;
    }

    SUBTEST_BEGIN(params, "invalid creation of group in read-only file")
    {
        /* Create a group with the read-only file handle (should fail) */
        H5E_BEGIN_TRY
        {
            group_id =
                H5Gcreate2(file_id, FILE_PERMISSION_TEST_GRP_NAME, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
        }
        H5E_END_TRY

        if (group_id >= 0) {
            printf("    a group was created in a read-only file!\n");
            TESTFRAME_TEST_ERROR(params);
        }

        H5E_BEGIN_TRY
        {
            group_id = H5Gcreate_anon(file_id, H5P_DEFAULT, H5P_DEFAULT);
        }
        H5E_END_TRY

        if (group_id >= 0) {
            printf("    a group was created in a read-only file!\n");
            TESTFRAME_TEST_ERROR(params);
        }
    }
    SUBTEST_END(params);

    SUBTEST_BEGIN(params, "invalid creation of dataset in read-only file")
    {
        /* Create a dataset with the read-only file handle (should fail) */
        H5E_BEGIN_TRY
        {
            dset_id = H5Dcreate2(file_id, FILE_PERMISSION_TEST_DSET2_NAME, H5T_STD_U32LE, dspace_id,
                                 H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
        }
        H5E_END_TRY

        if (dset_id >= 0) {
            printf("    a dataset was created in a read-only file!\n");
            TESTFRAME_TEST_ERROR(params);
        }

        H5E_BEGIN_TRY
        {
            dset_id = H5Dcreate_anon(file_id, H5T_STD_U32LE, dspace_id, H5P_DEFAULT, H5P_DEFAULT);
        }
        H5E_END_TRY

        if (dset_id >= 0) {
            printf("    a dataset was created in a read-only file!\n");
            TESTFRAME_TEST_ERROR(params);
        }
    }
    SUBTEST_END(params);

    SUBTEST_BEGIN(params, "invalid creation of attribute in read-only file")
    {
        /* Create an attribute with the read-only file handle (should fail) */
        H5E_BEGIN_TRY
        {
            attr_id = H5Acreate2(file_id, FILE_PERMISSION_TEST_ATTR_NAME, H5T_NATIVE_INT, dspace_id,
                                 H5P_DEFAULT, H5P_DEFAULT);
        }
        H5E_END_TRY

        if (attr_id >= 0) {
            printf("    an attribute was created in a read-only file!\n");
            TESTFRAME_TEST_ERROR(params);
        }
    }
    SUBTEST_END(params);

    SUBTEST_BEGIN(params, "invalid creation of committed datatype in read-only file")
    {
        if ((dtype_id = H5Tcopy(H5T_NATIVE_INT)) < 0) {
            printf("    couldn't copy a native datatype\n");
            TESTFRAME_TEST_ERROR(params);
        }

        /* Commit a datatype with the read-only file handle (should fail) */
        H5E_BEGIN_TRY
        {
            h5_ret = H5Tcommit2(file_id, FILE_PERMISSION_TEST_NAMED_DTYPE, dtype_id, H5P_DEFAULT, H5P_DEFAULT,
                                H5P_DEFAULT);
        }
        H5E_END_TRY

        if (h5_ret >= 0) {
            printf("    a named datatype was committed in a read-only file!\n");
            TESTFRAME_TEST_ERROR(params);
        }

        H5E_BEGIN_TRY
        {
            h5_ret = H5Tcommit_anon(file_id, dtype_id, H5P_DEFAULT, H5P_DEFAULT);
        }
        H5E_END_TRY

        if (h5_ret >= 0) {
            printf("    a named datatype was committed in a read-only file!\n");
            TESTFRAME_TEST_ERROR(params);
        }
    }
    SUBTEST_END(params);

    if (H5Tclose(dtype_id) < 0)
        TESTFRAME_TEST_ERROR(params);
    if (H5Sclose(dspace_id) < 0)
        TESTFRAME_TEST_ERROR(params);
    if (H5Fclose(file_id) < 0)
        TESTFRAME_TEST_ERROR(params);
    if (GetTestCleanup() && H5Fdelete(prefixed_filename, H5P_DEFAULT) < 0)
        TESTFRAME_TEST_ERROR(params);

    free(prefixed_filename);
    prefixed_filename = NULL;

    return SUCCEED;

error:
    H5E_BEGIN_TRY
    {
        H5Sclose(dspace_id);
        H5Dclose(dset_id);
        H5Aclose(attr_id);
        H5Tclose(dtype_id);
        H5Gclose(group_id);
        H5Fclose(file_id);
        if (GetTestCleanup())
            H5Fdelete(prefixed_filename, H5P_DEFAULT);
    }
    H5E_END_TRY

    free(prefixed_filename);

    return FAIL;
}

/*
 * A test to check that a file can be re-opened with H5Freopen.
 */
static herr_t
test_reopen_file(TestParams_t *params)
{
    hid_t file_id  = H5I_INVALID_HID;
    hid_t file_id2 = H5I_INVALID_HID;

    /* Make sure the connector supports the API functions being tested */
    if (!(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_BASIC)) {
        printf("    API functions for basic file aren't supported with this connector\n");
        return SKIP;
    }

    if ((file_id = H5Fopen(H5_API_TEST_FILENAME(params), H5F_ACC_RDWR, H5P_DEFAULT)) < 0) {
        printf("    couldn't open file\n");
        goto error;
    }

    if ((file_id2 = H5Freopen(file_id)) < 0) {
        printf("    couldn't re-open file\n");
        goto error;
    }

    if (H5Fclose(file_id) < 0)
        TESTFRAME_TEST_ERROR(params);
    if (H5Fclose(file_id2) < 0)
        TESTFRAME_TEST_ERROR(params);

    return SUCCEED;

error:
    H5E_BEGIN_TRY
    {
        H5Fclose(file_id);
        H5Fclose(file_id2);
    }
    H5E_END_TRY

    return FAIL;
}

/*
 * A test to check that H5Fclose doesn't succeed for an
 * invalid file ID */
static herr_t
test_close_file_invalid_id(TestParams_t H5_ATTR_UNUSED *params)
{
    herr_t err_ret = -1;

    /* Make sure the connector supports the API functions being tested */
    if (!(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_BASIC)) {
        printf("    API functions for basic file aren't supported with this connector\n");
        return SKIP;
    }

    H5E_BEGIN_TRY
    {
        err_ret = H5Fclose(H5I_INVALID_HID);
    }
    H5E_END_TRY

    if (err_ret >= 0) {
        printf("    closed an invalid file ID!\n");
        goto error;
    }

    return SUCCEED;

error:
    return FAIL;
}

/*
 * A test to check that a file can be flushed using H5Fflush.
 */
static herr_t
test_flush_file(TestParams_t *params)
{
    hid_t    file_id           = H5I_INVALID_HID;
    hid_t    dspace_id         = H5I_INVALID_HID;
    hid_t    dset_id           = H5I_INVALID_HID;
    char    *prefixed_filename = NULL;
    char     dset_name[32];
    unsigned u;

    /* Make sure the connector supports the API functions being tested */
    if (!(vol_cap_flags_g & (H5VL_CAP_FLAG_FILE_BASIC)) || !(vol_cap_flags_g & H5VL_CAP_FLAG_DATASET_BASIC) ||
        !(vol_cap_flags_g & H5VL_CAP_FLAG_FLUSH_REFRESH)) {
        printf("    API functions for basic file, dataset, or file flush aren't supported with this "
               "connector\n");
        return SKIP;
    }

    if (prefix_test_filename(params, test_path_prefix, FILE_FLUSH_TEST_FILENAME, &prefixed_filename) < 0) {
        printf("    couldn't prefix filename\n");
        goto error;
    }

    if ((file_id = H5Fcreate(prefixed_filename, H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        printf("    couldn't create file '%s'\n", prefixed_filename);
        goto error;
    }

    /* Create multiple small datasets in file */
    if ((dspace_id = H5Screate(H5S_SCALAR)) < 0) {
        printf("    couldn't create data space\n");
        goto error;
    }

    for (u = 0; u < 10; u++) {
        snprintf(dset_name, sizeof(dset_name), "Dataset %u", u);

        if ((dset_id = H5Dcreate2(file_id, dset_name, H5T_STD_U32LE, dspace_id, H5P_DEFAULT, H5P_DEFAULT,
                                  H5P_DEFAULT)) < 0) {
            printf("    couldn't create data set: %s\n", dset_name);
            goto error;
        }

        if (H5Dclose(dset_id) < 0)
            TESTFRAME_TEST_ERROR(params);
    }

    SUBTEST_BEGIN(params, "file flushing at local scope")
    {
        if (H5Fflush(file_id, H5F_SCOPE_LOCAL) < 0) {
            printf("    unable to flush file with scope H5F_SCOPE_LOCAL\n");
            TESTFRAME_TEST_ERROR(params);
        }
    }
    SUBTEST_END(params);

    SUBTEST_BEGIN(params, "file flushing at global scope")
    {
        if (H5Fflush(file_id, H5F_SCOPE_GLOBAL) < 0) {
            printf("    unable to flush file with scope H5F_SCOPE_GLOBAL\n");
            TESTFRAME_TEST_ERROR(params);
        }
    }
    SUBTEST_END(params);

    if (H5Sclose(dspace_id) < 0)
        TESTFRAME_TEST_ERROR(params);
    if (H5Fclose(file_id) < 0)
        TESTFRAME_TEST_ERROR(params);
    if (GetTestCleanup() && H5Fdelete(prefixed_filename, H5P_DEFAULT) < 0)
        TESTFRAME_TEST_ERROR(params);

    free(prefixed_filename);
    prefixed_filename = NULL;

    return SUCCEED;

error:
    H5E_BEGIN_TRY
    {
        H5Sclose(dspace_id);
        H5Dclose(dset_id);
        H5Fclose(file_id);
        if (GetTestCleanup())
            H5Fdelete(prefixed_filename, H5P_DEFAULT);
    }
    H5E_END_TRY

    free(prefixed_filename);

    return FAIL;
}

/*
 * A test for H5Fis_accessible.
 */
static herr_t
test_file_is_accessible(TestParams_t *params)
{
    const char *const fake_filename     = "nonexistent_file.h5";
    char             *prefixed_filename = NULL;
    htri_t            is_accessible     = FAIL;

    /* Make sure the connector supports the API functions being tested */
    if (!(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_BASIC)) {
        printf("    API functions for basic file aren't supported with this connector\n");
        return SKIP;
    }

    if (prefix_test_filename(params, test_path_prefix, fake_filename, &prefixed_filename) < 0) {
        printf("    couldn't prefix filename\n");
        goto error;
    }

    SUBTEST_BEGIN(params, "H5Fis_accessible on existing file")
    {
        if ((is_accessible = H5Fis_accessible(H5_API_TEST_FILENAME(params), H5P_DEFAULT)) < 0) {
            printf("    couldn't determine if file '%s' is accessible with default FAPL\n",
                   H5_API_TEST_FILENAME(params));
            TESTFRAME_TEST_ERROR(params);
        }

        if (!is_accessible) {
            printf("    file '%s' is not accessible with default FAPL\n", H5_API_TEST_FILENAME(params));
            TESTFRAME_TEST_ERROR(params);
        }
    }
    SUBTEST_END(params);

    is_accessible = -1;

    SUBTEST_BEGIN(params, "H5Fis_accessible on non-existing file")
    {
        H5E_BEGIN_TRY
        {
            is_accessible = H5Fis_accessible(prefixed_filename, H5P_DEFAULT);
        }
        H5E_END_TRY

        if (is_accessible > 0) {
            printf("    non-existent file '%s' was accessible with default FAPL: is_accessible=%d!\n",
                   prefixed_filename, is_accessible);
            TESTFRAME_TEST_ERROR(params);
        }
    }
    SUBTEST_END(params);

    free(prefixed_filename);
    prefixed_filename = NULL;

    return SUCCEED;

error:
    free(prefixed_filename);

    return FAIL;
}

/*
 * A test to check that a FCPL used for file creation can
 * be persisted and that a valid copy of that FCPL can be
 * retrieved later with a call to H5Fget_create_plist. Also
 * tests that a valid copy of a FAPL used for file access
 * can be retrieved with a call to H5Fget_access_plist.
 */
static herr_t
test_file_property_lists(TestParams_t *params)
{
    hsize_t prop_val           = 0;
    hid_t   file_id1           = H5I_INVALID_HID;
    hid_t   file_id2           = H5I_INVALID_HID;
    hid_t   fcpl_id1           = H5I_INVALID_HID;
    hid_t   fcpl_id2           = H5I_INVALID_HID;
    hid_t   fapl_id1           = H5I_INVALID_HID;
    hid_t   fapl_id2           = H5I_INVALID_HID;
    char   *prefixed_filename1 = NULL;
    char   *prefixed_filename2 = NULL;

    /* Make sure the connector supports the API functions being tested */
    if (!(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_BASIC) || !(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_MORE) ||
        !(vol_cap_flags_g & H5VL_CAP_FLAG_GET_PLIST)) {
        printf("    API functions for basic or more file or get property list aren't supported with this "
               "connector\n");
        return SKIP;
    }

    if (prefix_test_filename(params, test_path_prefix, FILE_PROPERTY_LIST_TEST_FNAME1, &prefixed_filename1) <
        0) {
        printf("    couldn't prefix filename\n");
        goto error;
    }
    if (prefix_test_filename(params, test_path_prefix, FILE_PROPERTY_LIST_TEST_FNAME2, &prefixed_filename2) <
        0) {
        printf("    couldn't prefix filename\n");
        goto error;
    }

    if ((fcpl_id1 = H5Pcreate(H5P_FILE_CREATE)) < 0) {
        printf("    couldn't create FCPL\n");
        goto error;
    }

    if (H5Pset_userblock(fcpl_id1, FILE_PROPERTY_LIST_TEST_FCPL_PROP_VAL) < 0) {
        printf("    failed to set test property on FCPL\n");
        goto error;
    }

    if ((file_id1 = H5Fcreate(prefixed_filename1, H5F_ACC_TRUNC, fcpl_id1, H5P_DEFAULT)) < 0) {
        printf("    couldn't create file\n");
        goto error;
    }

    if ((file_id2 = H5Fcreate(prefixed_filename2, H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        printf("    couldn't create file\n");
        goto error;
    }

    if (H5Pclose(fcpl_id1) < 0)
        TESTFRAME_TEST_ERROR(params);

    SUBTEST_BEGIN(params, "H5Fget_create_plist")
    {
        /* Try to receive copies of the two property lists, one which has the property set and one which
         * does not */
        if ((fcpl_id1 = H5Fget_create_plist(file_id1)) < 0) {
            printf("    couldn't get FCPL\n");
            TESTFRAME_TEST_ERROR(params);
        }

        if ((fcpl_id2 = H5Fget_create_plist(file_id2)) < 0) {
            printf("    couldn't get FCPL\n");
            TESTFRAME_TEST_ERROR(params);
        }

        /* Ensure that property list 1 has the property set and property list 2 does not */
        if (H5Pget_userblock(fcpl_id1, &prop_val) < 0) {
            printf("    failed to retrieve test property from FCPL\n");
            TESTFRAME_TEST_ERROR(params);
        }

        if (prop_val != FILE_PROPERTY_LIST_TEST_FCPL_PROP_VAL) {
            printf("    retrieved test property value '%llu' did not match expected value '%llu'\n",
                   (long long unsigned)prop_val, (long long unsigned)FILE_PROPERTY_LIST_TEST_FCPL_PROP_VAL);
            TESTFRAME_TEST_ERROR(params);
        }

        if (H5Pget_userblock(fcpl_id2, &prop_val) < 0) {
            printf("    failed to retrieve test property from FCPL\n");
            TESTFRAME_TEST_ERROR(params);
        }

        if (prop_val == FILE_PROPERTY_LIST_TEST_FCPL_PROP_VAL) {
            printf("    retrieved test property value '%llu' matched control value '%llu' when it "
                   "shouldn't have\n",
                   (long long unsigned)prop_val, (long long unsigned)FILE_PROPERTY_LIST_TEST_FCPL_PROP_VAL);
            TESTFRAME_TEST_ERROR(params);
        }
    }
    SUBTEST_END(params);

    SUBTEST_BEGIN(params, "H5Fget_access_plist")
    {
        /* Due to the nature of needing to supply a FAPL with the VOL connector having been set on it to
         * the H5Fcreate() call, we cannot exactly test using H5P_DEFAULT as the FAPL for one of the
         * create calls in this test. However, the use of H5Fget_access_plist() will still be used to
         * check that the FAPL is correct after both creating and opening a file.
         */
        if ((fapl_id1 = H5Fget_access_plist(file_id1)) < 0) {
            printf("    couldn't get FAPL\n");
            TESTFRAME_TEST_ERROR(params);
        }

        if ((fapl_id2 = H5Fget_access_plist(file_id2)) < 0) {
            printf("    couldn't get FAPL\n");
            TESTFRAME_TEST_ERROR(params);
        }
    }
    SUBTEST_END(params);

    /* Now see if we can still retrieve copies of the property lists upon opening
     * (instead of creating) a file. If they were reconstructed properly upon file
     * open, the creation property lists should also have the same test values
     * as set before.
     */
    if (fcpl_id1 >= 0) {
        H5E_BEGIN_TRY
        {
            H5Pclose(fcpl_id1);
        }
        H5E_END_TRY
        fcpl_id1 = H5I_INVALID_HID;
    }
    if (fcpl_id2 >= 0) {
        H5E_BEGIN_TRY
        {
            H5Pclose(fcpl_id2);
        }
        H5E_END_TRY
        fcpl_id2 = H5I_INVALID_HID;
    }
    if (fapl_id1 >= 0) {
        H5E_BEGIN_TRY
        {
            H5Pclose(fapl_id1);
        }
        H5E_END_TRY
        fapl_id1 = H5I_INVALID_HID;
    }
    if (fapl_id2 >= 0) {
        H5E_BEGIN_TRY
        {
            H5Pclose(fapl_id2);
        }
        H5E_END_TRY
        fapl_id2 = H5I_INVALID_HID;
    }
    if (file_id1 >= 0) {
        H5E_BEGIN_TRY
        {
            H5Fclose(file_id1);
        }
        H5E_END_TRY
        file_id1 = H5I_INVALID_HID;
    }
    if (file_id2 >= 0) {
        H5E_BEGIN_TRY
        {
            H5Fclose(file_id2);
        }
        H5E_END_TRY
        file_id2 = H5I_INVALID_HID;
    }

    SUBTEST_BEGIN(params, "H5Fget_create_plist after re-opening file")
    {
        if ((file_id1 = H5Fopen(prefixed_filename1, H5F_ACC_RDWR, H5P_DEFAULT)) < 0) {
            printf("    couldn't open file\n");
            TESTFRAME_TEST_ERROR(params);
        }

        if ((file_id2 = H5Fopen(prefixed_filename2, H5F_ACC_RDWR, H5P_DEFAULT)) < 0) {
            printf("    couldn't open file\n");
            TESTFRAME_TEST_ERROR(params);
        }

        if ((fcpl_id1 = H5Fget_create_plist(file_id1)) < 0) {
            printf("    couldn't get FCPL\n");
            TESTFRAME_TEST_ERROR(params);
        }

        if ((fcpl_id2 = H5Fget_create_plist(file_id2)) < 0) {
            printf("    couldn't get FCPL\n");
            TESTFRAME_TEST_ERROR(params);
        }

        /* Check the values of the test property */
        if (H5Pget_userblock(fcpl_id1, &prop_val) < 0) {
            printf("    failed to retrieve test property from FCPL\n");
            TESTFRAME_TEST_ERROR(params);
        }

        if (prop_val != FILE_PROPERTY_LIST_TEST_FCPL_PROP_VAL) {
            printf("    retrieved test property value '%llu' did not match expected value '%llu'\n",
                   (long long unsigned)prop_val, (long long unsigned)FILE_PROPERTY_LIST_TEST_FCPL_PROP_VAL);
            TESTFRAME_TEST_ERROR(params);
        }

        if (H5Pget_userblock(fcpl_id2, &prop_val) < 0) {
            printf("    failed to retrieve test property from FCPL\n");
            TESTFRAME_TEST_ERROR(params);
        }

        if (prop_val == FILE_PROPERTY_LIST_TEST_FCPL_PROP_VAL) {
            printf("    retrieved test property value '%llu' matched control value '%llu' when it "
                   "shouldn't have\n",
                   (long long unsigned)prop_val, (long long unsigned)FILE_PROPERTY_LIST_TEST_FCPL_PROP_VAL);
            TESTFRAME_TEST_ERROR(params);
        }
    }
    SUBTEST_END(params);

    if (H5Pclose(fcpl_id1) < 0)
        TESTFRAME_TEST_ERROR(params);
    if (H5Pclose(fcpl_id2) < 0)
        TESTFRAME_TEST_ERROR(params);
    if (H5Fclose(file_id1) < 0)
        TESTFRAME_TEST_ERROR(params);
    if (H5Fclose(file_id2) < 0)
        TESTFRAME_TEST_ERROR(params);
    if (GetTestCleanup() && H5Fdelete(prefixed_filename1, H5P_DEFAULT) < 0)
        TESTFRAME_TEST_ERROR(params);
    if (GetTestCleanup() && H5Fdelete(prefixed_filename2, H5P_DEFAULT) < 0)
        TESTFRAME_TEST_ERROR(params);

    free(prefixed_filename1);
    prefixed_filename1 = NULL;
    free(prefixed_filename2);
    prefixed_filename2 = NULL;

    return SUCCEED;

error:
    H5E_BEGIN_TRY
    {
        H5Pclose(fcpl_id1);
        H5Pclose(fcpl_id2);
        H5Pclose(fapl_id1);
        H5Pclose(fapl_id2);
        H5Fclose(file_id1);
        H5Fclose(file_id2);
        if (GetTestCleanup()) {
            H5Fdelete(prefixed_filename1, H5P_DEFAULT);
            H5Fdelete(prefixed_filename2, H5P_DEFAULT);
        }
    }
    H5E_END_TRY

    free(prefixed_filename1);
    free(prefixed_filename2);

    return FAIL;
}

/*
 * A test to check that the file intent flags can be retrieved.
 */
static herr_t
test_get_file_intent(TestParams_t *params)
{
    unsigned file_intent;
    hid_t    file_id           = H5I_INVALID_HID;
    char    *prefixed_filename = NULL;

    /* Make sure the connector supports the API functions being tested */
    if (!(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_BASIC) || !(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_MORE)) {
        printf("    API functions for basic or more file aren't supported with this connector\n");
        return SKIP;
    }

    if (prefix_test_filename(params, test_path_prefix, FILE_INTENT_TEST_FILENAME, &prefixed_filename) < 0) {
        printf("    couldn't prefix filename\n");
        goto error;
    }

    /* Test that file intent retrieval works correctly for file create */
    if ((file_id = H5Fcreate(prefixed_filename, H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        printf("    couldn't create file '%s'\n", prefixed_filename);
        goto error;
    }

    SUBTEST_BEGIN(params, "H5Fget_intent on newly-created file")
    {
        if (H5Fget_intent(file_id, &file_intent) < 0) {
            printf("    failed to retrieve file intent\n");
            TESTFRAME_TEST_ERROR(params);
        }

        if (H5F_ACC_RDWR != file_intent) {
            printf("    received incorrect file intent for file creation\n");
            TESTFRAME_TEST_ERROR(params);
        }
    }
    SUBTEST_END(params);

    if (file_id >= 0) {
        H5E_BEGIN_TRY
        {
            H5Fclose(file_id);
        }
        H5E_END_TRY
        file_id = H5I_INVALID_HID;
    }

    SUBTEST_BEGIN(params, "H5Fget_intent for file opened read-only")
    {
        /* Test that file intent retrieval works correctly for file open */
        if ((file_id = H5Fopen(H5_API_TEST_FILENAME(params), H5F_ACC_RDONLY, H5P_DEFAULT)) < 0) {
            printf("    couldn't open file '%s'\n", H5_API_TEST_FILENAME(params));
            TESTFRAME_TEST_ERROR(params);
        }

        if (H5Fget_intent(file_id, &file_intent) < 0) {
            printf("    failed to retrieve file intent\n");
            TESTFRAME_TEST_ERROR(params);
        }

        if (H5F_ACC_RDONLY != file_intent) {
            printf("    received incorrect file intent for read-only file open\n");
            TESTFRAME_TEST_ERROR(params);
        }
    }
    SUBTEST_END(params);

    if (file_id >= 0) {
        H5E_BEGIN_TRY
        {
            H5Fclose(file_id);
        }
        H5E_END_TRY
        file_id = H5I_INVALID_HID;
    }

    SUBTEST_BEGIN(params, "H5Fget_intent for file opened read-write")
    {
        if ((file_id = H5Fopen(H5_API_TEST_FILENAME(params), H5F_ACC_RDWR, H5P_DEFAULT)) < 0) {
            printf("    couldn't open file '%s'\n", H5_API_TEST_FILENAME(params));
            TESTFRAME_TEST_ERROR(params);
        }

        if (H5Fget_intent(file_id, &file_intent) < 0) {
            printf("    failed to retrieve file intent\n");
            TESTFRAME_TEST_ERROR(params);
        }

        if (H5F_ACC_RDWR != file_intent) {
            printf("    received incorrect file intent\n");
            TESTFRAME_TEST_ERROR(params);
        }
    }
    SUBTEST_END(params);

    if (file_id >= 0) {
        H5E_BEGIN_TRY
        {
            H5Fclose(file_id);
        }
        H5E_END_TRY
        file_id = H5I_INVALID_HID;
    }

    if (GetTestCleanup() && H5Fdelete(prefixed_filename, H5P_DEFAULT) < 0)
        TESTFRAME_TEST_ERROR(params);

    free(prefixed_filename);
    prefixed_filename = NULL;

    return SUCCEED;

error:
    H5E_BEGIN_TRY
    {
        H5Fclose(file_id);
        if (GetTestCleanup())
            H5Fdelete(prefixed_filename, H5P_DEFAULT);
    }
    H5E_END_TRY

    free(prefixed_filename);

    return FAIL;
}

/*
 * A test to check that the number of open objects and IDs of objects in a file
 * can be retrieved.
 */
static herr_t
test_get_file_obj_count(TestParams_t *params)
{
    ssize_t obj_count;
    hid_t   file_id            = H5I_INVALID_HID;
    hid_t   file_id2           = H5I_INVALID_HID;
    hid_t   group_id           = H5I_INVALID_HID;
    hid_t   object_id          = H5I_INVALID_HID;
    hid_t   named_dtype_id     = H5I_INVALID_HID;
    hid_t   attr_id            = H5I_INVALID_HID;
    hid_t   dspace_id          = H5I_INVALID_HID;
    hid_t   dset_id            = H5I_INVALID_HID;
    char   *prefixed_filename1 = NULL;
    char   *prefixed_filename2 = NULL;

    /* Make sure the connector supports the API functions being tested */
    if (!(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_BASIC) || !(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_MORE) ||
        !(vol_cap_flags_g & H5VL_CAP_FLAG_DATASET_BASIC) || !(vol_cap_flags_g & H5VL_CAP_FLAG_GROUP_BASIC) ||
        !(vol_cap_flags_g & H5VL_CAP_FLAG_STORED_DATATYPES) ||
        !(vol_cap_flags_g & H5VL_CAP_FLAG_ATTR_BASIC)) {
        printf(
            "    API functions for basic or more file,  basic dataset, group, stored datatypes, or attribute "
            "aren't supported with this connector\n");
        return SKIP;
    }

    if (prefix_test_filename(params, test_path_prefix, GET_OBJ_COUNT_TEST_FILENAME1, &prefixed_filename1) <
        0) {
        printf("    couldn't prefix filename\n");
        goto error;
    }
    if (prefix_test_filename(params, test_path_prefix, GET_OBJ_COUNT_TEST_FILENAME2, &prefixed_filename2) <
        0) {
        printf("    couldn't prefix filename\n");
        goto error;
    }

    if ((file_id = H5Fcreate(prefixed_filename1, H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        printf("    couldn't create file '%s'\n", prefixed_filename1);
        goto error;
    }

    if ((group_id = H5Gcreate2(file_id, GET_OBJ_COUNT_TEST_GRP_NAME, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) <
        0) {
        printf("    couldn't create group '%s'\n", GET_OBJ_COUNT_TEST_GRP_NAME);
        goto error;
    }

    /* Create a second file while keeping the first file open */
    if ((file_id2 = H5Fcreate(prefixed_filename2, H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        printf("    couldn't create file '%s'\n", prefixed_filename2);
        goto error;
    }

    /* Create a named datatype */
    if ((named_dtype_id = H5Tcopy(H5T_NATIVE_INT)) < 0) {
        printf("    couldn't copy a native datatype\n");
        goto error;
    }

    if (H5Tcommit2(file_id2, GET_OBJ_COUNT_TEST_NAMED_DTYPE, named_dtype_id, H5P_DEFAULT, H5P_DEFAULT,
                   H5P_DEFAULT) < 0) {
        printf("    couldn't commit a named datatype\n");
        goto error;
    }

    /* Create a dataspace for the attribute and dataset */
    if ((dspace_id = H5Screate(H5S_SCALAR)) < 0) {
        printf("    couldn't create data space for attribute\n");
        goto error;
    }

    /* Create an attribute for the second file */
    if ((attr_id = H5Acreate2(file_id2, GET_OBJ_COUNT_TEST_ATTR_NAME, H5T_NATIVE_INT, dspace_id, H5P_DEFAULT,
                              H5P_DEFAULT)) < 0) {
        printf("    couldn't create the attribute '%s'\n", GET_OBJ_COUNT_TEST_ATTR_NAME);
        goto error;
    }

    /* Create a dataset for the second file */
    if ((dset_id = H5Dcreate2(file_id2, GET_OBJ_COUNT_TEST_DSET_NAME, H5T_NATIVE_INT, dspace_id, H5P_DEFAULT,
                              H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        printf("    couldn't create the dataset '%s'\n", GET_OBJ_COUNT_TEST_DSET_NAME);
        goto error;
    }

    SUBTEST_BEGIN(params, "H5Fget_obj_count for files")
    {
        /* Get the number of files currently opened */
        if ((obj_count = H5Fget_obj_count((hid_t)H5F_OBJ_ALL, H5F_OBJ_FILE)) < 0) {
            printf("    couldn't get the number of open files\n");
            TESTFRAME_TEST_ERROR(params);
        }

        if (check_open_obj_count(obj_count, 2) < 0) {
            printf("    number of open files (%ld) did not match %s expected number (2)\n", obj_count,
                   H5_API_TEST_EXECUTION_CONCURRENT ? "or exceed" : "");
            TESTFRAME_TEST_ERROR(params);
        }
    }
    SUBTEST_END(params);

    SUBTEST_BEGIN(params, "H5Fget_obj_count for groups in single file")
    {
        /* Get the number of groups */
        if ((obj_count = H5Fget_obj_count(file_id, H5F_OBJ_GROUP)) < 0) {
            printf("    couldn't retrieve number of open groups\n");
            TESTFRAME_TEST_ERROR(params);
        }

        /* Only querying single file, no need to accomodate for possible multi-threading */
        if (obj_count != 1) {
            printf("    number of open groups (%ld) did not match expected number (1)\n", obj_count);
            TESTFRAME_TEST_ERROR(params);
        }
    }
    SUBTEST_END(params);

    SUBTEST_BEGIN(params, "H5Fget_obj_count for groups")
    {
        /* Get the number of groups in two opened files */
        if ((obj_count = H5Fget_obj_count((hid_t)H5F_OBJ_ALL, H5F_OBJ_GROUP)) < 0) {
            printf("    couldn't get the number of open groups\n");
            TESTFRAME_TEST_ERROR(params);
        }

        if (check_open_obj_count(obj_count, 1) < 0) {
            printf("    number of open groups (%ld) did not match %s expected number (1)\n", obj_count,
                   H5_API_TEST_EXECUTION_CONCURRENT ? "or exceed" : "");
            TESTFRAME_TEST_ERROR(params);
        }
    }
    SUBTEST_END(params);

    SUBTEST_BEGIN(params, "H5Fget_obj_count for datatypes")
    {
        /* Get the number of named datatype in two opened files */
        if ((obj_count = H5Fget_obj_count((hid_t)H5F_OBJ_ALL, H5F_OBJ_DATATYPE)) < 0) {
            printf("    couldn't get the number of open named datatypes\n");
            TESTFRAME_TEST_ERROR(params);
        }

        if (check_open_obj_count(obj_count, 1) < 0) {
            printf("    number of open named datatypes (%ld) did not match %s expected number (1)\n",
                   obj_count, H5_API_TEST_EXECUTION_CONCURRENT ? "or exceed" : "");
            TESTFRAME_TEST_ERROR(params);
        }
    }
    SUBTEST_END(params);

    SUBTEST_BEGIN(params, "H5Fget_obj_count for attributes")
    {
        /* Get the number of attribute in two opened files */
        if ((obj_count = H5Fget_obj_count((hid_t)H5F_OBJ_ALL, H5F_OBJ_ATTR)) < 0) {
            printf("    couldn't get the number of open attributes\n");
            TESTFRAME_TEST_ERROR(params);
        }

        if (check_open_obj_count(obj_count, 1) < 0) {
            printf("    number of open attributes (%ld) did not match %s expected number (1)\n", obj_count,
                   H5_API_TEST_EXECUTION_CONCURRENT ? "or exceed" : "");
            TESTFRAME_TEST_ERROR(params);
        }
    }
    SUBTEST_END(params);

    SUBTEST_BEGIN(params, "H5Fget_obj_count for datasets")
    {
        /* Get the number of dataset in two opened files */
        if ((obj_count = H5Fget_obj_count((hid_t)H5F_OBJ_ALL, H5F_OBJ_DATASET)) < 0) {
            printf("    couldn't get the number of open datasets\n");
            TESTFRAME_TEST_ERROR(params);
        }

        if (check_open_obj_count(obj_count, 1) < 0) {
            printf("    number of open datasets (%ld) did not match %s expected number (1)\n", obj_count,
                   H5_API_TEST_EXECUTION_CONCURRENT ? "or exceed" : "");
            TESTFRAME_TEST_ERROR(params);
        }
    }
    SUBTEST_END(params);

    SUBTEST_BEGIN(params, "H5Fget_obj_count for all object types in single file")
    {
        /* Get the number of all open objects */
        if ((obj_count = H5Fget_obj_count(file_id, H5F_OBJ_ALL)) < 0) {
            printf("    couldn't retrieve number of open objects\n");
            TESTFRAME_TEST_ERROR(params);
        }

        /* One for the file and another for the group */
        if (obj_count != 2) {
            printf("    number of open objects (%ld) did not match expected number (2)\n", obj_count);
            TESTFRAME_TEST_ERROR(params);
        }
    }
    SUBTEST_END(params);

    SUBTEST_BEGIN(params, "H5Fget_obj_count for all object types")
    {
        /* Get the number of all open objects */
        if ((obj_count = H5Fget_obj_count(H5F_OBJ_ALL, H5F_OBJ_ALL)) < 0) {
            printf("    couldn't retrieve number of open objects\n");
            TESTFRAME_TEST_ERROR(params);
        }

        if (check_open_obj_count(obj_count, 6) < 0) {
            printf("    number of open objects (%ld) did not match %s expected number (6)\n", obj_count,
                   H5_API_TEST_EXECUTION_CONCURRENT ? "or exceed" : "");
            TESTFRAME_TEST_ERROR(params);
        }
    }
    SUBTEST_END(params);

    SUBTEST_BEGIN(params, "H5Fget_obj_ids for a singular group")
    {
        if (H5Fget_obj_ids(file_id, H5F_OBJ_GROUP, (size_t)obj_count, &object_id) < 0) {
            printf("    couldn't get opened group IDs\n");
            TESTFRAME_TEST_ERROR(params);
        }

        if (object_id != group_id) {
            printf("    opened object ID (%ld) did not match only currently open group ID (%ld)\n", object_id,
                   group_id);
            TESTFRAME_TEST_ERROR(params);
        }
    }
    SUBTEST_END(params);

    if (H5Gclose(group_id) < 0)
        TESTFRAME_TEST_ERROR(params);
    if (H5Sclose(dspace_id) < 0)
        TESTFRAME_TEST_ERROR(params);
    if (H5Aclose(attr_id) < 0)
        TESTFRAME_TEST_ERROR(params);
    if (H5Tclose(named_dtype_id) < 0)
        TESTFRAME_TEST_ERROR(params);
    if (H5Dclose(dset_id) < 0)
        TESTFRAME_TEST_ERROR(params);
    if (H5Fclose(file_id) < 0)
        TESTFRAME_TEST_ERROR(params);
    if (H5Fclose(file_id2) < 0)
        TESTFRAME_TEST_ERROR(params);
    if (GetTestCleanup() && H5Fdelete(prefixed_filename1, H5P_DEFAULT) < 0)
        TESTFRAME_TEST_ERROR(params);
    if (GetTestCleanup() && H5Fdelete(prefixed_filename2, H5P_DEFAULT) < 0)
        TESTFRAME_TEST_ERROR(params);

    free(prefixed_filename1);
    prefixed_filename1 = NULL;
    free(prefixed_filename2);
    prefixed_filename2 = NULL;

    return SUCCEED;

error:
    H5E_BEGIN_TRY
    {
        H5Gclose(group_id);
        H5Tclose(named_dtype_id);
        H5Sclose(dspace_id);
        H5Aclose(attr_id);
        H5Dclose(dset_id);
        H5Fclose(file_id);
        H5Fclose(file_id2);
        if (GetTestCleanup()) {
            H5Fdelete(prefixed_filename1, H5P_DEFAULT);
            H5Fdelete(prefixed_filename2, H5P_DEFAULT);
        }
    }
    H5E_END_TRY

    free(prefixed_filename1);
    free(prefixed_filename2);

    return FAIL;
}

/*
 * A test to check that opening files in an overlapping way
 * works correctly.
 */
static herr_t
test_file_open_overlap(TestParams_t *params)
{
    ssize_t obj_count;
    hid_t   file_id           = H5I_INVALID_HID;
    hid_t   file_id2          = H5I_INVALID_HID;
    hid_t   group_id          = H5I_INVALID_HID;
    hid_t   dspace_id         = H5I_INVALID_HID;
    hid_t   dset_id           = H5I_INVALID_HID;
    char   *prefixed_filename = NULL;

    /* Make sure the connector supports the API functions being tested */
    if (!(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_BASIC) || !(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_MORE) ||
        !(vol_cap_flags_g & H5VL_CAP_FLAG_DATASET_BASIC) || !(vol_cap_flags_g & H5VL_CAP_FLAG_GROUP_BASIC)) {
        printf("    API functions for basic or more file, dataset, or group aren't supported with this "
               "connector\n");
        return SKIP;
    }

    if (prefix_test_filename(params, test_path_prefix, OVERLAPPING_FILENAME, &prefixed_filename) < 0) {
        printf("    couldn't prefix filename\n");
        goto error;
    }

    if ((file_id = H5Fcreate(prefixed_filename, H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        printf("    couldn't create file '%s'\n", prefixed_filename);
        goto error;
    }

    if ((file_id2 = H5Fopen(prefixed_filename, H5F_ACC_RDWR, H5P_DEFAULT)) < 0) {
        printf("    couldn't open file '%s'\n", prefixed_filename);
        goto error;
    }

    if ((group_id = H5Gcreate2(file_id, OVERLAPPING_OPEN_TEST_GRP_NAME, H5P_DEFAULT, H5P_DEFAULT,
                               H5P_DEFAULT)) < 0) {
        printf("    couldn't create group '%s'\n", OVERLAPPING_OPEN_TEST_GRP_NAME);
        goto error;
    }

    /* Create a dataspace for the dataset */
    if ((dspace_id = H5Screate(H5S_SCALAR)) < 0) {
        printf("    couldn't create data space for dataset\n");
        goto error;
    }

    /* Create a dataset in the group of the first file */
    if ((dset_id = H5Dcreate2(group_id, OVERLAPPING_OPEN_TEST_DSET_NAME, H5T_NATIVE_INT, dspace_id,
                              H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        printf("    couldn't create the dataset '%s'\n", OVERLAPPING_OPEN_TEST_DSET_NAME);
        goto error;
    }

    /* Get the number of objects opened in the first file: 3 == file + dataset + group */
    if ((obj_count = H5Fget_obj_count(file_id, H5F_OBJ_LOCAL | H5F_OBJ_ALL)) < 0) {
        printf("    couldn't retrieve the number of objects opened in the file\n");
        goto error;
    }

    if (obj_count != 3) {
        printf("    number of objects opened in file (%ld) did not match expected number (3)\n", obj_count);
        goto error;
    }

    if (H5Gclose(group_id) < 0)
        TESTFRAME_TEST_ERROR(params);
    if (H5Dclose(dset_id) < 0)
        TESTFRAME_TEST_ERROR(params);
    if (H5Fclose(file_id) < 0)
        TESTFRAME_TEST_ERROR(params);

    /* Create a dataset in the second file */
    if ((dset_id = H5Dcreate2(file_id2, OVERLAPPING_OPEN_TEST_DSET_NAME, H5T_NATIVE_INT, dspace_id,
                              H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        printf("    couldn't create the dataset '%s'\n", OVERLAPPING_OPEN_TEST_DSET_NAME);
        goto error;
    }

    /* Get the number of objects opened in the first file: 2 == file + dataset */
    if ((obj_count = H5Fget_obj_count(file_id2, H5F_OBJ_ALL)) < 0) {
        printf("    couldn't retrieve the number of objects opened in the file\n");
        goto error;
    }

    if (obj_count != 2) {
        printf("    number of objects opened in the file (%ld) did not match expected number (2)\n",
               obj_count);
        goto error;
    }

    if (H5Sclose(dspace_id) < 0)
        TESTFRAME_TEST_ERROR(params);
    if (H5Dclose(dset_id) < 0)
        TESTFRAME_TEST_ERROR(params);
    if (H5Fclose(file_id2) < 0)
        TESTFRAME_TEST_ERROR(params);
    if (GetTestCleanup() && H5Fdelete(prefixed_filename, H5P_DEFAULT) < 0)
        TESTFRAME_TEST_ERROR(params);

    free(prefixed_filename);
    prefixed_filename = NULL;

    return SUCCEED;

error:
    H5E_BEGIN_TRY
    {
        H5Gclose(group_id);
        H5Sclose(dspace_id);
        H5Dclose(dset_id);
        H5Fclose(file_id);
        H5Fclose(file_id2);
        if (GetTestCleanup())
            H5Fdelete(prefixed_filename, H5P_DEFAULT);
    }
    H5E_END_TRY

    free(prefixed_filename);

    return FAIL;
}

/*
 * A test to check that file mounting and unmounting works
 * correctly.
 */
static herr_t
test_file_mounts(TestParams_t *params)
{
    hid_t file_id           = H5I_INVALID_HID;
    hid_t child_fid         = H5I_INVALID_HID;
    hid_t group_id          = H5I_INVALID_HID;
    char *prefixed_filename = NULL;

    /* Make sure the connector supports the API functions being tested */
    if (!(vol_cap_flags_g & (H5VL_CAP_FLAG_FILE_BASIC)) || !(vol_cap_flags_g & H5VL_CAP_FLAG_MOUNT) ||
        !(vol_cap_flags_g & H5VL_CAP_FLAG_GROUP_BASIC)) {
        printf("    API functions for basic file,  file mount, or basic group aren't supported with this "
               "connector\n");
        return SKIP;
    }

    if (prefix_test_filename(params, test_path_prefix, FILE_MOUNT_TEST_FILENAME, &prefixed_filename) < 0) {
        printf("    couldn't prefix filename\n");
        goto error;
    }

    if ((file_id = H5Fcreate(prefixed_filename, H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        printf("    couldn't create file '%s'\n", prefixed_filename);
        goto error;
    }

    if ((group_id = H5Gcreate2(file_id, FILE_MOUNT_TEST_GRP_NAME, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) <
        0) {
        printf("    couldn't create group '%s'\n", FILE_MOUNT_TEST_GRP_NAME);
        goto error;
    }

    if ((child_fid = H5Fopen(H5_API_TEST_FILENAME(params), H5F_ACC_RDWR, H5P_DEFAULT)) < 0) {
        printf("    couldn't open file '%s'\n", H5_API_TEST_FILENAME(params));
        goto error;
    }

    /* Mount one file (child_fid) to the group of another file (file_id) */
    if (H5Fmount(file_id, FILE_MOUNT_TEST_GRP_NAME, child_fid, H5P_DEFAULT) < 0) {
        printf("    couldn't mount file\n");
        goto error;
    }

    if (H5Funmount(file_id, FILE_MOUNT_TEST_GRP_NAME) < 0) {
        printf("    couldn't mount file\n");
        goto error;
    }

    if (H5Gclose(group_id) < 0)
        TESTFRAME_TEST_ERROR(params);
    if (H5Fclose(file_id) < 0)
        TESTFRAME_TEST_ERROR(params);
    if (H5Fclose(child_fid) < 0)
        TESTFRAME_TEST_ERROR(params);
    if (GetTestCleanup() && H5Fdelete(prefixed_filename, H5P_DEFAULT) < 0)
        TESTFRAME_TEST_ERROR(params);

    free(prefixed_filename);
    prefixed_filename = NULL;

    return SUCCEED;

error:
    H5E_BEGIN_TRY
    {
        H5Gclose(group_id);
        H5Fclose(file_id);
        H5Fclose(child_fid);
        if (GetTestCleanup())
            H5Fdelete(prefixed_filename, H5P_DEFAULT);
    }
    H5E_END_TRY

    free(prefixed_filename);

    return FAIL;
}

/*
 * A test to ensure that a file's name can be retrieved.
 */
static herr_t
test_get_file_name(TestParams_t *params)
{
    ssize_t file_name_buf_len = 0;
    hid_t   file_id           = H5I_INVALID_HID;
    hid_t   group_id          = H5I_INVALID_HID;
    hid_t   dset_id           = H5I_INVALID_HID;
    hid_t   dspace_id         = H5I_INVALID_HID;
    hid_t   attr_id           = H5I_INVALID_HID;
    hid_t   named_dtype_id    = H5I_INVALID_HID;
    char   *prefixed_filename = NULL;
    char   *file_name_buf     = NULL;

    /* Make sure the connector supports the API functions being tested */
    if (!(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_BASIC) || !(vol_cap_flags_g & H5VL_CAP_FLAG_FILE_MORE) ||
        !(vol_cap_flags_g & H5VL_CAP_FLAG_DATASET_BASIC) || !(vol_cap_flags_g & H5VL_CAP_FLAG_GROUP_BASIC) ||
        !(vol_cap_flags_g & H5VL_CAP_FLAG_STORED_DATATYPES) ||
        !(vol_cap_flags_g & H5VL_CAP_FLAG_ATTR_BASIC)) {
        printf(
            "    API functions for basic or more file, basic dataset, group, stored datatypes, or attribute "
            "aren't supported with this connector\n");
        return SKIP;
    }

    if (prefix_test_filename(params, test_path_prefix, GET_FILE_NAME_TEST_FNAME, &prefixed_filename) < 0) {
        printf("    couldn't prefix filename\n");
        goto error;
    }

    if ((file_id = H5Fcreate(prefixed_filename, H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        printf("    couldn't create file '%s'\n", prefixed_filename);
        goto error;
    }

    /* Retrieve the size of the file name */
    if ((file_name_buf_len = H5Fget_name(file_id, NULL, 0)) < 0)
        TESTFRAME_TEST_ERROR(params);

    /* Allocate buffer for file name */
    if (NULL == (file_name_buf = (char *)malloc((size_t)file_name_buf_len + 1)))
        TESTFRAME_TEST_ERROR(params);

    SUBTEST_BEGIN(params, "H5Fget_name using file ID")
    {
        memset(file_name_buf, 0, (size_t)file_name_buf_len);

        /* Retrieve the actual file name */
        if (H5Fget_name(file_id, file_name_buf, (size_t)file_name_buf_len + 1) < 0) {
            printf("    couldn't get file name %s\n", prefixed_filename);
            TESTFRAME_TEST_ERROR(params);
        }

        if (strncmp(file_name_buf, prefixed_filename, (size_t)file_name_buf_len)) {
            printf("    file name '%s' didn't match expected name '%s'\n", file_name_buf, prefixed_filename);
            TESTFRAME_TEST_ERROR(params);
        }
    }
    SUBTEST_END(params);

    SUBTEST_BEGIN(params, "H5Fget_name using non-root group ID")
    {
        /* Attempt to retrieve the name of the file from an object that isn't the root group */
        memset(file_name_buf, 0, (size_t)file_name_buf_len);

        if ((group_id = H5Gcreate2(file_id, GET_FILE_NAME_TEST_GRP_NAME, H5P_DEFAULT, H5P_DEFAULT,
                                   H5P_DEFAULT)) < 0) {
            printf("    failed to create group '%s'\n", GET_FILE_NAME_TEST_GRP_NAME);
            TESTFRAME_TEST_ERROR(params);
        }

        if (H5Fget_name(group_id, file_name_buf, (size_t)file_name_buf_len + 1) < 0) {
            printf("    couldn't get file name %s\n", prefixed_filename);
            TESTFRAME_TEST_ERROR(params);
        }

        if (strncmp(file_name_buf, prefixed_filename, (size_t)file_name_buf_len)) {
            printf("    file name '%s' didn't match expected name '%s'\n", file_name_buf, prefixed_filename);
            TESTFRAME_TEST_ERROR(params);
        }

        if (group_id >= 0) {
            H5E_BEGIN_TRY
            {
                H5Gclose(group_id);
            }
            H5E_END_TRY
            group_id = H5I_INVALID_HID;
        }
    }
    SUBTEST_END(params);

    SUBTEST_BEGIN(params, "H5Fget_name using dataset ID")
    {
        if ((dspace_id = H5Screate(H5S_SCALAR)) < 0) {
            printf("    couldn't create dataspace\n");
            TESTFRAME_TEST_ERROR(params);
        }

        /* Create a dataset in the file */
        if ((dset_id = H5Dcreate2(file_id, GET_FILE_NAME_TEST_DSET_NAME, H5T_NATIVE_INT, dspace_id,
                                  H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
            printf("    couldn't create the dataset '%s'\n", GET_FILE_NAME_TEST_DSET_NAME);
            TESTFRAME_TEST_ERROR(params);
        }

        /* Get and verify file name from the dataset */
        if (H5Fget_name(dset_id, file_name_buf, (size_t)file_name_buf_len + 1) < 0) {
            printf("    couldn't get file name %s\n", prefixed_filename);
            TESTFRAME_TEST_ERROR(params);
        }

        if (strncmp(file_name_buf, prefixed_filename, (size_t)file_name_buf_len)) {
            printf("    file name '%s' didn't match expected name '%s'\n", file_name_buf, prefixed_filename);
            TESTFRAME_TEST_ERROR(params);
        }

        if (dspace_id >= 0) {
            H5E_BEGIN_TRY
            {
                H5Sclose(dspace_id);
            }
            H5E_END_TRY
            dspace_id = H5I_INVALID_HID;
        }
        if (dset_id >= 0) {
            H5E_BEGIN_TRY
            {
                H5Dclose(dset_id);
            }
            H5E_END_TRY
            dset_id = H5I_INVALID_HID;
        }
    }
    SUBTEST_END(params);

    SUBTEST_BEGIN(params, "H5Fget_name using attribute ID")
    {
        if ((dspace_id = H5Screate(H5S_SCALAR)) < 0) {
            printf("    couldn't create dataspace\n");
            TESTFRAME_TEST_ERROR(params);
        }

        /* Create an attribute for the dataset */
        if ((attr_id = H5Acreate2(file_id, GET_FILE_NAME_TEST_ATTR_NAME, H5T_NATIVE_INT, dspace_id,
                                  H5P_DEFAULT, H5P_DEFAULT)) < 0) {
            printf("    couldn't create the attribute '%s'\n", GET_FILE_NAME_TEST_ATTR_NAME);
            TESTFRAME_TEST_ERROR(params);
        }

        /* Get and verify file name from the attribute */
        if (H5Fget_name(attr_id, file_name_buf, (size_t)file_name_buf_len + 1) < 0) {
            printf("    couldn't get file name %s\n", prefixed_filename);
            TESTFRAME_TEST_ERROR(params);
        }

        if (strncmp(file_name_buf, prefixed_filename, (size_t)file_name_buf_len)) {
            printf("    file name '%s' didn't match expected name '%s'\n", file_name_buf, prefixed_filename);
            TESTFRAME_TEST_ERROR(params);
        }

        if (dspace_id >= 0) {
            H5E_BEGIN_TRY
            {
                H5Sclose(dspace_id);
            }
            H5E_END_TRY
            dspace_id = H5I_INVALID_HID;
        }
        if (attr_id >= 0) {
            H5E_BEGIN_TRY
            {
                H5Aclose(attr_id);
            }
            H5E_END_TRY
            attr_id = H5I_INVALID_HID;
        }
    }
    SUBTEST_END(params);

    SUBTEST_BEGIN(params, "H5Fget_name using committed datatype ID")
    {
        /* Create a named datatype */
        if ((named_dtype_id = H5Tcopy(H5T_NATIVE_INT)) < 0) {
            printf("    couldn't copy a native datatype\n");
            TESTFRAME_TEST_ERROR(params);
        }

        if (H5Tcommit2(file_id, GET_FILE_NAME_TEST_NAMED_DTYPE, named_dtype_id, H5P_DEFAULT, H5P_DEFAULT,
                       H5P_DEFAULT) < 0) {
            printf("    couldn't commit a named datatype\n");
            TESTFRAME_TEST_ERROR(params);
        }

        /* Get and verify file name from the committed datatype */
        if (H5Fget_name(named_dtype_id, file_name_buf, (size_t)file_name_buf_len + 1) < 0) {
            printf("    couldn't get file name %s\n", prefixed_filename);
            TESTFRAME_TEST_ERROR(params);
        }

        if (strncmp(file_name_buf, prefixed_filename, (size_t)file_name_buf_len)) {
            printf("    file name '%s' didn't match expected name '%s'\n", file_name_buf, prefixed_filename);
            TESTFRAME_TEST_ERROR(params);
        }

        if (named_dtype_id >= 0) {
            H5E_BEGIN_TRY
            {
                H5Tclose(named_dtype_id);
            }
            H5E_END_TRY
            named_dtype_id = H5I_INVALID_HID;
        }
    }
    SUBTEST_END(params);

    SUBTEST_BEGIN(params, "invalid H5Fget_name using dataspace ID")
    {
        ssize_t name_len = 0;

        if ((dspace_id = H5Screate(H5S_SCALAR)) < 0) {
            printf("    couldn't create dataspace\n");
            TESTFRAME_TEST_ERROR(params);
        }

        /* Try get file name from data space.  Supposed to fail because
         * it's illegal operation. */
        H5E_BEGIN_TRY
        {
            name_len = H5Fget_name(dspace_id, file_name_buf, (size_t)file_name_buf_len + 1);
        }
        H5E_END_TRY

        if (name_len >= 0) {
            printf("    retrieved file name using H5Fget_name on a dataspace ID!\n");
            TESTFRAME_TEST_ERROR(params);
        }

        if (dspace_id >= 0) {
            H5E_BEGIN_TRY
            {
                H5Sclose(dspace_id);
            }
            H5E_END_TRY
            dspace_id = H5I_INVALID_HID;
        }
    }
    SUBTEST_END(params);

    if (file_name_buf) {
        free(file_name_buf);
        file_name_buf = NULL;
    }

    if (H5Fclose(file_id) < 0)
        TESTFRAME_TEST_ERROR(params);
    if (GetTestCleanup() && H5Fdelete(prefixed_filename, H5P_DEFAULT) < 0)
        TESTFRAME_TEST_ERROR(params);

    free(prefixed_filename);
    prefixed_filename = NULL;

    return SUCCEED;

error:
    H5E_BEGIN_TRY
    {
        if (file_name_buf)
            free(file_name_buf);
        H5Tclose(named_dtype_id);
        H5Sclose(dspace_id);
        H5Dclose(dset_id);
        H5Aclose(attr_id);
        H5Gclose(group_id);
        H5Fclose(file_id);
        if (GetTestCleanup())
            H5Fdelete(prefixed_filename, H5P_DEFAULT);
    }
    H5E_END_TRY

    free(prefixed_filename);

    return FAIL;
}

/*
 * Helper to check that the number of recorded open objects in the entire library is as expected.
 * This is only necessary when H5Fget_obj_count()'s first argument is H5F_OBJ_ALL.
 *
 * If the API tests are being run in a single thread, this is a wrapper around a direct comparison.
 *
 * If the API tests are being run in multiple threads concurrently, then the
 * number of open objects could be any value at least as large as the number of objects opened by this thread
 * - the best we can do is verify it's not definitely wrong.
 */
herr_t
check_open_obj_count(ssize_t obj_count, int expected)
{
    herr_t ret_value = SUCCEED;
    /* If multiple threads are concurrently executing tests, then more objects than expected may be open in
     * the library */
    if (H5_API_TEST_EXECUTION_CONCURRENT && obj_count < expected) {
        ret_value = FAIL;
    }
    else if (!H5_API_TEST_EXECUTION_CONCURRENT &&
             obj_count != expected) { /* Single thread, expect exact count */
        ret_value = FAIL;
    }

    return ret_value;
}

void
H5_api_file_test_add(void)
{
    uint64_t testframe_flags = ALLOW_MULTITHREAD;

    AddTest("test_create_file", test_create_file, NULL, NULL, NULL, 0, testframe_flags, "H5Fcreate");

    /* Add a header to the first file test to distinguish different test interfaces */
    AddTestHeaderFunc("test_create_file", print_file_test_header);

    AddTest("test_create_file_invalid_params", test_create_file_invalid_params, NULL, NULL, NULL, 0,
            testframe_flags, "H5Fcreate with invalid parameters");
    AddTest("test_create_file_excl", test_create_file_excl, NULL, NULL, NULL, 0, testframe_flags,
            "H5Fcreate with H5F_ACC_EXCL/H5F_ACC_TRUNC flag");
    AddTest("test_open_file", test_open_file, NULL, NULL, NULL, 0, testframe_flags, "H5Fopen");
    AddTest("test_open_file_invalid_params", test_open_file_invalid_params, NULL, NULL, NULL, 0,
            testframe_flags, "H5Fopen with invalid parameters");
    AddTest("test_open_nonexistent_file", test_open_nonexistent_file, NULL, NULL, NULL, 0, testframe_flags,
            "for invalid opening of a non-existent file");
    AddTest("test_file_open_overlap", test_file_open_overlap, NULL, NULL, NULL, 0, testframe_flags,
            "overlapping file opens");
    AddTest("test_file_permission", test_file_permission, NULL, NULL, NULL, 0, testframe_flags,
            "file permissions (invalid creation of objects in read-only file)");
    AddTest("test_reopen_file", test_reopen_file, NULL, NULL, NULL, 0, testframe_flags,
            "re-open of a file with H5Freopen");
    AddTest("test_close_file_invalid_id", test_close_file_invalid_id, NULL, NULL, NULL, 0, testframe_flags,
            "H5Fclose with an invalid ID");
    AddTest("test_flush_file", test_flush_file, NULL, NULL, NULL, 0, testframe_flags, "H5Fflush");
    AddTest("test_file_is_accessible", test_file_is_accessible, NULL, NULL, NULL, 0, testframe_flags,
            "H5Fis_accessible");
    AddTest("test_file_property_lists", test_file_property_lists, NULL, NULL, NULL, 0, testframe_flags,
            "file property list operations");
    AddTest("test_get_file_intent", test_get_file_intent, NULL, NULL, NULL, 0, testframe_flags,
            "retrieval of file intent with H5Fget_intent");
    AddTest("test_get_file_obj_count", test_get_file_obj_count, NULL, NULL, NULL, 0, testframe_flags,
            "retrieval of open object number and IDs");
    AddTest("test_file_mounts", test_file_mounts, NULL, NULL, NULL, 0, testframe_flags,
            "file mounting/unmounting");
    AddTest("test_get_file_name", test_get_file_name, NULL, NULL, NULL, 0, testframe_flags,
            "retrieval of file name");
}